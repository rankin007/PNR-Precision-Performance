...
function New-Case([string]$Name) {
  ...
  [IO.File]::WriteAllText((Join-Path $dir '0021_postgresql_filename_extension_parser_correction.sql'),($parser -join "`n"),$utf8); $dir
}
...
try {
  ...
  if($result.Migrations.Count-ne 21-or$result.Diagnostic-notmatch 'Candidate repository'-or$result.Diagnostic-match '(?i)applied successfully|remote applied'){throw 'valid diagnostic failed'}
  $missing=New-Case missing; Get-ChildItem $missing -Filter '0021*'|Remove-Item; Must-Fail {Test-CandidateMigrationLedger $missing} missing
  ...
  $renamed=New-Case renamed; Get-ChildItem $renamed -Filter '0021*'|Rename-Item -NewName '0021_other.sql'; Must-Fail {Test-CandidateMigrationLedger $renamed} renamed
  $empty=New-Case empty; [IO.File]::WriteAllText((Join-Path $empty '0021_postgresql_filename_extension_parser_correction.sql'),'',$utf8); Must-Fail {Test-CandidateMigrationLedger $empty} placeholder
  $future=New-Case future; [IO.File]::WriteAllText((Join-Path $future '0022_future.sql'),'select 1;',$utf8); Must-Fail {Test-CandidateMigrationLedger $future} future
  ...
  $unrelated=New-Case unrelated; [IO.File]::WriteAllText((Join-Path $unrelated 'README.sql'),'notes',$utf8); if((Test-CandidateMigrationLedger $unrelated).Migrations.Count-ne 21){throw 'unrelated file affected ledger'}
