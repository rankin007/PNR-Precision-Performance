...
function New-Case([string]$Name) {
  ...
  [IO.File]::WriteAllText((Join-Path $dir '0022_public_trainer_enquiries.sql'),'select 1;',$utf8); $dir
}
...
try {
  ...
  if($result.Migrations.Count-ne 22){throw 'valid migration count failed'}; Pass
  if($result.Diagnostic-notmatch 'aligned through 0022'){throw 'valid repository diagnostic failed'}; Pass
  ...
  $missing=New-Case missing; Get-ChildItem $missing -Filter '0022*'|Remove-Item; Must-Fail {Test-CandidateMigrationLedger $missing} missing; Pass
  ...
  $future=New-Case future; [IO.File]::WriteAllText((Join-Path $future '0023_future.sql'),'select 1;',$utf8); Must-Fail {Test-CandidateMigrationLedger $future} future; Pass
  ...
  $unrelated=New-Case unrelated; [IO.File]::WriteAllText((Join-Path $unrelated 'README.sql'),'notes',$utf8); if((Test-CandidateMigrationLedger $unrelated).Migrations.Count-ne 22){throw 'unrelated file affected ledger'}; Pass
