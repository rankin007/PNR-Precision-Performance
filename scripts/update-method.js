...
function readManifest(root) {
  try {
    ...
    return {
      ...
      localCorrections:
        parsed.localCorrections && typeof parsed.localCorrections === "object"
          ? parsed.localCorrections
          : {},
    };
  } catch {
    return { version: null, mode: null, source: null, files: {}, localCorrections: {} };
  }
}
...
async function run({ cwd = process.cwd(), fetchImpl = globalThis.fetch, log = console.log } = {}) {
  ...
  const localCorrections = manifest.localCorrections || {};
  const retainLocalCorrections =
    Number.isFinite(current.version) &&
    Number.isFinite(manifest.version) &&
    current.version <= manifest.version &&
    Object.keys(localCorrections).length > 0;
  const effectiveCurrent = {
    ...current,
    files: { ...current.files },
  };
  if (retainLocalCorrections) {
    for (const [relativePath, expectedHash] of Object.entries(localCorrections)) {
      if (
        !isSafeRelativePath(relativePath) ||
        typeof expectedHash !== "string" ||
        !Object.prototype.hasOwnProperty.call(current.files, relativePath) ||
        manifest.files[relativePath] !== expectedHash ||
        folderHashes[relativePath] !== expectedHash
      ) {
        throw new Error(`Local method correction drifted or left the source allowlist: ${relativePath}`);
      }
      effectiveCurrent.files[relativePath] = expectedHash;
    }
  }
  ...
  const diff = diffManifest(effectiveCurrent, folderHashes, manifest.version, {
      ...
      JSON.stringify(
        {
          ...
          source: base,
          files: effectiveCurrent.files,
          ...(retainLocalCorrections ? { localCorrections } : {}),
        },
        null,
        2
      ) + "\n";
