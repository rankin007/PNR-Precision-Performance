...
equal(TARGET_KEY_NAME, "Precision Performance public enquiry 029U", "target key name exact");
...
const controlSnapshot = (pageClass, controls) => ({
  ...
  protected: {
    ...
    token: ["re", "syntheticprotectedtoken029u12345"].join("_"),
  },
...
if (assertions !== 16) throw new Error(`Sprint 029U provider projection assertion target changed: ${assertions}/16`);
console.log(`Sprint 029U identity-blind provider and sensitivity projection tests passed (${assertions}/${assertions}).`);
