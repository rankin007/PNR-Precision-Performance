...
$liveTarget = 'PrecisionPerformance/029Q/PreflightBearer'
$testTarget = 'PrecisionPerformance/029Q/SyntheticTest'
$remoteNames = @(
...
public static class PP029QCredential {
...
function Write-FixedCredential([string]$Target, [string]$PlainText) {
  ...
  try {
    $credential = New-Object PP029QCredential+CREDENTIAL
    $credential.Type = 1
    $credential.TargetName = $Target
    $credential.Persist = 2
    $credential.CredentialBlob = $blob
    $credential.CredentialBlobSize = [Text.Encoding]::Unicode.GetByteCount($PlainText)
    $credential.UserName = 'sprint-029q'
    if (-not [PP029QCredential]::Write([ref]$credential, 0)) { throw 'CREDENTIAL_WRITE_REFUSED' }
  } finally {
...
function Read-FixedCredential([string]$Target) {
  $pointer = [IntPtr]::Zero
  if (-not [PP029QCredential]::Read($Target, 1, 0, [ref]$pointer)) { throw 'CREDENTIAL_ABSENT' }
  try {
    $credential = [Runtime.InteropServices.Marshal]::PtrToStructure($pointer, [type][PP029QCredential+CREDENTIAL])
...
function Remove-FixedCredential([string]$Target) {
  if (-not [PP029QCredential]::Delete($Target, 1, 0)) {
...
function Read-LiveRecord {
  ...
  try {
    ...
    if ($record.version -ne 1 -or $record.remoteNames.Count -ne 3 -or @($record.remoteNames | Where-Object { $_ -notin $remoteNames }).Count -ne 0) { throw 'CREDENTIAL_RECORD_REFUSED' }
    return $record
  } finally {
    $plain = $null
  }
...
function Assert-Origin([string]$Value) {
  ...
  if ($uri.Scheme -cne 'https' -or $uri.AbsolutePath -cne '/' -or $uri.Query -or $uri.Fragment -or $uri.Host -notmatch '^pnr-precision-performance-[a-z0-9]+-rankin007s-projects\.vercel\.app$') { throw 'ORIGIN_REFUSED' }
...
if ($Operation -eq 'SelfTest') {
  ...
  '{"controller":"029Q","operation":"self-test","state":"pass","credentialResidue":0}'
  exit 0
}
...
if ($Operation -eq 'Provision') {
  ...
  try {
    ...
    $record = [ordered]@{ version=1; runId=[guid]::NewGuid().ToString('N'); bearer=$bearer; verifier=$verifier; notBefore=(Format-CanonicalUtc $notBefore); expiresAt=(Format-CanonicalUtc $expiresAt); remoteNames=$remoteNames }
    ...
    '{"controller":"029Q","operation":"provision","state":"pass","bindingCount":3,"windowClass":"bounded","credentialState":"present"}'
  } catch {
...
if ($Operation -eq 'Status') {
  ...
  [pscustomobject]@{controller='029Q';operation='status';state='pass';bindingCount=$count;credentialState=if(Test-FixedCredential $liveTarget){'present'}else{'absent'}} | ConvertTo-Json -Compress
  exit 0
}
...
try {
  if ($Operation -eq 'VerifyReady') {
    ...
    if ($current -lt [DateTime]::Parse($record.notBefore).ToUniversalTime() -or $current -ge [DateTime]::Parse($record.expiresAt).ToUniversalTime()) { throw 'WINDOW_REFUSED' }
    ...
    if ($value.result -cne 'smtp-preflight' -or $value.status -cne 'ready' -or $value.providerClass -cne 'resend' -or $null -ne $value.errorClass) { throw 'PREFLIGHT_REFUSED' }
    '{"controller":"029Q","operation":"verify-ready","state":"pass","requestCount":1,"providerClass":"resend","errorClass":null}'
    ...
    '{"controller":"029Q","operation":"expired-denial-cleanup","state":"pass","requestCount":1,"httpClass":"not-found","bindingResidue":0,"credentialResidue":0}'
  } elseif ($Operation -eq 'Compensate') {
    Remove-OwnedTemporaryResources $record
    '{"controller":"029Q","operation":"compensate","state":"pass","bindingResidue":0,"credentialResidue":0}'
  }
} finally {
