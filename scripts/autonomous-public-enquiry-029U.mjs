...
} from "./provider-browser-projection-029U.mjs";

export const PROJECT_ID = "prj_6To7czLpCEGL6fInkQwE4egePPpq";
export const BRANCH = "codex/029U-operator-isolated-guarded-smtp-readiness-recovery";
...
export const IMPLEMENTATION_FILES = Object.freeze([
  ...
  "scripts/test-public-enquiry-029P.mjs",
...
export const INHERITED_DEPLOYMENT_FILES = Object.freeze([
  ...
  "scripts/autonomous-public-enquiry-029P.mjs",
...
export function validateBaselineProjection(value) {
  if (!exactKeys(value, BASELINE_KEYS)
    || value.controller !== "029U" || value.operation !== "baseline" || value.state !== "pass"
...
const SELF_TEST_KEYS = Object.freeze([
  ...
  "deploymentTargetRefusalCount", "baselineFixturePassCount", "baselineFailureCodeCount", "runnerResidue",
]);

export function validateControllerSelfTest(value) {
  if (!exactKeys(value, SELF_TEST_KEYS)
    || value.controller !== "029U" || value.operation !== "self-test" || value.state !== "pass"
    || value.allowedVectorCount !== 21 || value.refusedVectorCount !== 16
    ...
    || value.baselineFailureCodeCount !== 8 || value.runnerResidue !== 0) fail("PROJECTION_REFUSED");
...
export function validatePrivatePasswordProjection(value) {
  if (!exactKeys(value, PRIVATE_PASSWORD_KEYS)
    || value.controller !== "029U" || value.operation !== "accept-private-password-baseline" || value.state !== "pass"
...
export function validateStructuralProjection(value) {
  if (!exactKeys(value, STRUCTURAL_KEYS)
    || value.controller !== "029U" || value.operation !== "add-structural-smtp" || value.state !== "pass"
...
export function validateProvisionProjection(value) {
  if (!exactKeys(value, PROVISION_KEYS)
    || value.controller !== "029U" || value.operation !== "provision" || value.state !== "pass"
...
export function validatePublicGateProjection(value) {
  if (!exactKeys(value, PUBLIC_GATE_KEYS)
    || value.controller !== "029U" || value.operation !== "verify-public-gate" || value.state !== "pass"
...
const DEPLOYMENT_KEYS = Object.freeze([
  ...
  "aliasCount", "metadataClass", "sourceClass",
]);

export function validateDeploymentProjection(value) {
  if (!exactKeys(value, DEPLOYMENT_KEYS)
    || value.controller !== "029U" || value.operation !== "deploy" || value.state !== "pass"
    || !DEPLOYMENT_ID.test(value.deploymentId) || value.targetClass !== "production"
    || value.readinessClass !== "ready" || value.aliasCount !== 0
    || value.metadataClass !== "exact-029U" || value.sourceClass !== "exact-canonical") fail("DEPLOYMENT_REFUSED");
  return { deploymentId: value.deploymentId, aliasCount: 0, targetClass: "production" };
}
