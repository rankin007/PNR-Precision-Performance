...
param(
  [Parameter(Mandatory)]
  [ValidateSet('SelfTest','CredentialSelfTest','Baseline','AcceptPrivatePasswordBaseline','AddStructuralSmtp','Provision','Deploy','VerifyPublicGate','VerifyReady','VerifyExpiredAndCleanup','Status','Compensate')]
  [string]$Operation,
  [string]$Origin = ''
)
...
$expectedBranch = 'codex/029U-operator-isolated-guarded-smtp-readiness-recovery'
...
$liveTarget = 'PrecisionPerformance/029U/PreflightBearer'
$testTarget = 'PrecisionPerformance/029U/SyntheticTest'
...
$baselineFailureCodes = @(
  ...
  'BASELINE_029U_DEPLOYMENT_REFUSED',
...
public static class PP029UCredential {
...
function Set-FixedCredential([string]$Target, [string]$PlainText, [bool]$AllowOverwrite = $false) {
  ...
  try {
    $credential = New-Object PP029UCredential+CREDENTIAL
    $credential.Type = 1
    $credential.TargetName = $Target
    $credential.Persist = 2
    $credential.CredentialBlob = $blob
    $credential.CredentialBlobSize = [Text.Encoding]::Unicode.GetByteCount($PlainText)
    $credential.UserName = 'sprint-029u'
    if (-not [PP029UCredential]::Write([ref]$credential, 0)) { throw 'CREDENTIAL_WRITE_REFUSED' }
  } finally {
...
function Read-FixedCredential([string]$Target) {
  $pointer = [IntPtr]::Zero
  if (-not [PP029UCredential]::Read($Target, 1, 0, [ref]$pointer)) { throw 'CREDENTIAL_ABSENT' }
  try {
    $credential = [Runtime.InteropServices.Marshal]::PtrToStructure($pointer, [type][PP029UCredential+CREDENTIAL])
...
function Remove-FixedCredential([string]$Target) {
  if (-not [PP029UCredential]::Delete($Target, 1, 0)) {
...
function Assert-AllowedVercelVector {
  ...
  $fixed = @(
    ...
    @{ Class='deployment-list'; Args=@('list','--meta','pp_sprint=029U','--format','json','--no-color'); Input=$false },
    @{ Class='deploy'; Args=@('deploy','--prod','--skip-domain','--yes','--format','json','--no-color','--meta','pp_sprint=029U'); Input=$false }
  )
...
function Get-DeploymentListProjection {
  $result = Invoke-VercelCaptured -CommandArgs @('list','--meta','pp_sprint=029U','--format','json','--no-color')
...
function ConvertTo-DeploymentProjection {
  ...
  return [pscustomobject]@{
    ...
    MetadataClass=$(if($meta -and $meta.PSObject.Properties.Name -ccontains 'pp_sprint' -and $meta.pp_sprint -ceq '029U'){'exact-029U'}else{'historical'})
...
function Assert-BaselineProjection {
  ...
  if ($SprintDeploymentCount -ne 0) { Stop-BaselineFailure 'BASELINE_029U_DEPLOYMENT_REFUSED' }
...
function Invoke-ControllerSelfTest {
  ...
  $allowed.Add(@{ Args=@('list','--meta','pp_sprint=029U','--format','json','--no-color'); Input=$false })
  $allowed.Add(@{ Args=@('deploy','--prod','--skip-domain','--yes','--format','json','--no-color','--meta','pp_sprint=029U'); Input=$false })
  ...
  $processTransfer = Invoke-CapturedChildProcess -ExecutablePath $env:ComSpec -CommandArgs @('/d','/s','/c','echo','PP029U_ONE','PP029U_TWO','PP029U_THREE','PP029U_FOUR') -InputWasBound $false -InputValue $null -VectorClass 'self-test-process'
  if ($processTransfer.ExitCode -ne 0 -or $processTransfer.VectorClass -cne 'self-test-process' -or
    $processTransfer.Stdout.Trim() -cne 'PP029U_ONE PP029U_TWO PP029U_THREE PP029U_FOUR' -or $processTransfer.Stderr) { throw 'SELF_TEST_REFUSED' }
  ...
  try {
    [void](ConvertTo-DeploymentProjection -Value ([pscustomobject]@{ id=$inert029NDeployment; name='synthetic'; url='synthetic.example'; target='production'; readyState='ERROR'; aliases=@() }) -DeploymentId $inert029NDeployment)
  ...
  $emptyDeploymentProjection = @(ConvertTo-DeploymentListProjection -Rows $emptyDeploymentRows)
  ...
  foreach ($failureCode in $baselineFailureCodes) {
    ...
    switch -CaseSensitive ($failureCode) {
      ...
      'BASELINE_029U_DEPLOYMENT_REFUSED' { $deploymentCountFixture = 1 }
  ...
  $refused = @(
    ...
    @('deploy','--prod','--skip-domain','--yes','--format','json','--no-color','--meta','pp_sprint=029U','--force'),
    @('deploy','--prod','--yes','--format','json','--no-color','--meta','pp_sprint=029U'),
...
if ($Operation -eq 'CredentialSelfTest') {
  ...
  '{"controller":"029U","operation":"credential-self-test","state":"pass","credentialResidue":0}'
  exit 0
}
...
if ($Operation -eq 'Baseline') {
  ...
  try { $sprintDeployments = Get-DeploymentListProjection } catch { Stop-BaselineFailure 'BASELINE_029U_DEPLOYMENT_REFUSED' }
  ...
  [pscustomobject]@{controller='029U';operation='baseline';state='pass';projectClass='exact';dedicatedSmtpRowCount=0;temporaryAuthRowCount=0;activationRowCount=0;genericSmtpRowCount=5;credentialState='absent';sprintDeploymentCount=0;retainedPreviewClass='ready-preview-inert';retainedPreviewAliasCount=0;priorCandidateCount=$priorCandidateCount;priorCandidateAliasCount=0;acceptedDeploymentClass='ready';acceptedAliasTargetCount=5} | ConvertTo-Json -Compress
  exit 0
}
...
if ($Operation -eq 'AcceptPrivatePasswordBaseline') {
  ...
  '{"controller":"029U","operation":"accept-private-password-baseline","state":"pass","dedicatedSmtpRowCount":1,"temporaryAuthRowCount":0,"activationRowCount":0,"genericSmtpRowCount":5,"passwordSensitivityClass":"exact-sensitive-production","sprintDeploymentCount":0,"acceptedAliasTargetCount":5}'
  exit 0
}
...
if ($Operation -eq 'AddStructuralSmtp') {
  ...
  '{"controller":"029U","operation":"add-structural-smtp","state":"pass","dedicatedSmtpRowCount":4,"temporaryAuthRowCount":0,"activationRowCount":0,"sensitivityClass":"all-exact-sensitive-production"}'
  exit 0
}
...
if ($Operation -eq 'Provision') {
  ...
  try {
    ...
    '{"controller":"029U","operation":"provision","state":"pass","bindingCount":3,"windowClass":"bounded","credentialState":"present","activationRowCount":0,"sensitivityClass":"all-seven-exact-sensitive-production"}'
  } catch {
    $operationFailure = $_.Exception.Message
...
if ($Operation -eq 'Deploy') {
  ...
  try {
    ...
    $environment = Get-EnvironmentProjection
    if ($environment.Dedicated -ne 4 -or $environment.Temporary -ne 3 -or $environment.Activation -ne 0 -or $environment.Generic -ne 5 -or (Get-DeploymentListProjection).Count -ne 0) { throw 'BASELINE_REFUSED' }
    $deployed = Invoke-VercelCaptured -CommandArgs @('deploy','--prod','--skip-domain','--yes','--format','json','--no-color','--meta','pp_sprint=029U')
    ...
    if ($inspection.TargetClass -ne 'production' -or $inspection.ReadinessClass -ne 'ready' -or $inspection.AliasCount -ne 0 -or
      $inspection.MetadataClass -ne 'exact-029U' -or $inspection.SourceClass -ne 'exact-canonical') { throw 'DEPLOYMENT_REFUSED' }
    $record.deploymentId = $id
    Set-FixedCredential $liveTarget ($record | ConvertTo-Json -Compress) $true
    [pscustomobject]@{controller='029U';operation='deploy';state='pass';deploymentId=$id;targetClass='production';readinessClass='ready';aliasCount=0;metadataClass='exact-029U';sourceClass='exact-canonical'} | ConvertTo-Json -Compress
...
if ($Operation -eq 'Status') {
  ...
  if ($deploymentIds.Count -eq 1) {
    ...
    if ($candidate.TargetClass -ne 'production' -or $candidate.MetadataClass -ne 'exact-029U') { throw 'DEPLOYMENT_REFUSED' }
...
if ($Operation -eq 'Compensate') {
  ...
  try {
    ...
    $after = Get-EnvironmentProjection
    if ($after.Dedicated -ne 0 -or $after.Temporary -ne 0 -or $after.Activation -ne 0 -or (Test-FixedCredential $liveTarget)) { throw 'CLEANUP_REFUSED' }
    '{"controller":"029U","operation":"compensate","state":"pass","bindingResidue":0,"credentialResidue":0}'
  } finally { if($record){$record.bearer=$null;$record.verifier=$null};$record=$null }
  exit 0
}
...
try {
  if ($Operation -eq 'VerifyPublicGate') {
    ...
    $result = Invoke-PublicGate $candidateOrigin
    ...
    '{"controller":"029U","operation":"verify-public-gate","state":"pass","requestCount":1,"httpClass":"service-unavailable","responseClass":"sanitized","productActionCount":0}'
    ...
    '{"controller":"029U","operation":"verify-ready","state":"pass","requestCount":1,"providerClass":"resend","errorClass":null}'
    ...
    '{"controller":"029U","operation":"expired-denial-cleanup","state":"pass","requestCount":1,"httpClass":"not-found","bindingResidue":0,"credentialResidue":0}'
  }
} finally {
