...
export function sanitizedAuthorizedRemovalEvidence(input) {
  ...
  assert.equal(evidence.disposition, REMOVED_AS_OBSOLETE, "035Q requires the authorised obsolete-removal disposition");
  return evidence;
}
...
export function assertSpecificErrorSummary(input) {
  assert.equal(input.summaryCount, 1, "expected exactly one specific error summary");
  assert.equal(input.id, "biochemistry-error-summary", "specific error summary id changed");
  assert.equal(input.role, "alert", "specific error summary must expose role=alert");
  assert.equal(input.visible, true, "specific error summary must remain visible");
  assert.match(input.text ?? "", /correct.+highlighted fields.+before review/i, "specific error summary must provide actionable correction guidance");
  assert.equal(input.focused, true, "specific error summary must receive focus");
  assert.ok(Number.isInteger(input.pageAlertCount) && input.pageAlertCount >= 1, "page must expose at least the specific alert");
  return true;
}

const HOSTED_RUN = "035Q-RENDERED-20260802-01";
...
async function runHostedRenderedChecks() {
  const [{ createClient }, { createServerClient }] = await Promise.all([
  ...
  try {
    ...
    result = await service.from("stables").insert({ name: `${HOSTED_RUN}-STABLE-A`, code: "S035QA", status: "active" }).select("id").single();
    assert.ifError(result.error); owned.stableA = result.data.id;
    result = await service.from("stables").insert({ name: `${HOSTED_RUN}-STABLE-B`, code: "S035QB", status: "active" }).select("id").single();
    assert.ifError(result.error); owned.stableB = result.data.id;
    result = await service.from("horses").insert({ stable_id: owned.stableA, name: `${HOSTED_RUN}-ASSIGNED`, slug: "035q-assigned", status: "active", breed: "Synthetic thoroughbred", colour: "Synthetic bay", date_of_birth: "2020-01-01" }).select("id").single();
    assert.ifError(result.error); owned.horseA = result.data.id;
    result = await service.from("horses").insert({ stable_id: owned.stableB, name: `${HOSTED_RUN}-DENIED`, slug: "035q-denied", status: "active", breed: "Synthetic thoroughbred", colour: "Synthetic bay", date_of_birth: "2020-01-01" }).select("id").single();
    ...
    const errorSummary = page.locator("#biochemistry-error-summary");
    await errorSummary.waitFor({ state: "visible", timeout: 5000 });
    await page.waitForFunction(() => document.activeElement?.id === "biochemistry-error-summary", undefined, { timeout: 5000 });
    assertSpecificErrorSummary({
      summaryCount: await errorSummary.count(),
      id: await errorSummary.getAttribute("id"),
      role: await errorSummary.getAttribute("role"),
      text: await errorSummary.innerText(),
      visible: await errorSummary.isVisible(),
      focused: await errorSummary.evaluate((element) => document.activeElement === element),
      pageAlertCount: await page.locator('[role="alert"]').count(),
    });
    pass(7, "specific validation summary exposes actionable alert guidance");
  ...
  console.log(JSON.stringify({ event: "035q-rendered-result", passed, expected: 14, failure: failure ?? "none", cleanup: { authRemaining, applicationRemaining, storageRemaining: 0 } }));
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href && process.argv[2] === "hosted") {
  runHostedRenderedChecks().catch((error) => {
    console.error(`Sprint 035Q hosted proof stopped safely: ${error.message}`);
    process.exitCode = 1;
  });
}
