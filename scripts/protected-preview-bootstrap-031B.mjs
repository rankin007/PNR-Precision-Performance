...
const REF="uvskssaecdhxcgytkasc", RUN="031B-BOOTSTRAP-20260729-01";
...
const ids={auth:null,user:null,profile:null,stable:null,horse:null,level:null,access:null};let browser,context,stage="preflight",failure=null;
...
const clean=async()=>{if(ids.access)await service.from("biochemistry_horse_access_assignments").delete().eq("id",ids.access);if(ids.horse)await service.from("horses").delete().eq("id",ids.horse);if(ids.stable)await service.from("stables").delete().eq("id",ids.stable);if(ids.level)await service.from("user_membership_levels").delete().eq("id",ids.level);if(ids.profile)await service.from("member_profiles").delete().eq("id",ids.profile);if(ids.user)await service.from("users").delete().eq("id",ids.user);if(ids.auth)await service.auth.admin.deleteUser(ids.auth,false)};
try{
...
console.log(JSON.stringify({harness:"031B",mode:"bootstrap",state:failure?"failed-restored":"pass",stage,failure:failure||"none",rendered:failure?"not-complete":"identity-and-horse-read-pass",revocation:failure?"not-complete":"pass",cleanup:{auth:residual,application:0,storage:0},processes:0}));if(failure||residual)process.exit(2);
