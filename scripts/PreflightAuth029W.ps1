...
$expectedBranch = 'codex/029W-vercel-inventory-projection-and-final-readiness-attempt'
...
$liveTarget = 'PrecisionPerformance/029W/PreflightBearer'
$testTarget = 'PrecisionPerformance/029W/SyntheticTest'
$script:OwnedDeploymentId = ''
$script:OwnedDeploymentHost = ''
$inventoryProjectorSha = 'ad04b8711775ecbedd0acd36102455b9745bc52490a9f480efc0e02f3b5af142'
...
$baselineFailureCodes = @(
  ...
  'BASELINE_029W_DEPLOYMENT_REFUSED',
...
public static class PP029WCredential {
...
function Set-FixedCredential([string]$Target, [string]$PlainText, [bool]$AllowOverwrite = $false) {
  ...
  try {
    $credential = New-Object PP029WCredential+CREDENTIAL
    $credential.Type = 1
    $credential.TargetName = $Target
    $credential.Persist = 2
    $credential.CredentialBlob = $blob
    $credential.CredentialBlobSize = [Text.Encoding]::Unicode.GetByteCount($PlainText)
    $credential.UserName = 'sprint-029w'
    if (-not [PP029WCredential]::Write([ref]$credential, 0)) { throw 'CREDENTIAL_WRITE_REFUSED' }
  } finally {
...
function Read-FixedCredential([string]$Target) {
  $pointer = [IntPtr]::Zero
  if (-not [PP029WCredential]::Read($Target, 1, 0, [ref]$pointer)) { throw 'CREDENTIAL_ABSENT' }
  try {
    $credential = [Runtime.InteropServices.Marshal]::PtrToStructure($pointer, [type][PP029WCredential+CREDENTIAL])
...
function Remove-FixedCredential([string]$Target) {
  if (-not [PP029WCredential]::Delete($Target, 1, 0)) {
...
function Assert-AllowedVercelVector {
  ...
  $fixed = @(
    ...
    @{ Class='deploy'; Args=@('deploy','--prod','--skip-domain','--yes','--format','json','--no-color','--meta','pp_sprint=029W'); Input=$false }
  )
  ...
  if ($CommandArgs.Count -eq 5 -and $CommandArgs[0] -ceq 'inspect' -and
    ...
    if ($script:OwnedDeploymentHost -and $CommandArgs[1] -ceq $script:OwnedDeploymentHost -and
      (Test-StrictDeploymentHostname $CommandArgs[1])) { return 'inspect-owned-host' }
  }
...
function Test-StrictDeploymentHostname([string]$Value) {
  if (-not $Value -or $Value.Length -gt 253 -or $Value -cne $Value.ToLowerInvariant() -or
    -not $Value.EndsWith('.vercel.app') -or $Value -match '[:/?#]') { return $false }
...
function ConvertTo-DeploymentInventoryProjection {
  param([Parameter(Mandatory)][pscustomobject]$Value)
  Assert-ExactJsonObject -Value $Value -AllowedFields @('projectionVersion','rows','pagination') -RequiredFields @('projectionVersion','rows','pagination') -RefusalCode 'INVENTORY_PROJECTION_REFUSED'
  if (($Value.projectionVersion -isnot [int] -and $Value.projectionVersion -isnot [long])) { throw 'INVENTORY_PROJECTION_REFUSED' }
  if ([long]$Value.projectionVersion -ne 1 -or $Value.rows -isnot [array]) { throw 'INVENTORY_PROJECTION_VERSION_REFUSED' }
  Assert-ExactJsonObject -Value $Value.pagination -AllowedFields @('count','next','prev') -RequiredFields @('count','next','prev') -RefusalCode 'INVENTORY_PROJECTION_REFUSED'
  if (($Value.pagination.count -isnot [int] -and $Value.pagination.count -isnot [long]) -or
    $Value.pagination.count -lt 0 -or $Value.pagination.count -gt 20 -or $Value.pagination.count -ne @($Value.rows).Count) { throw 'INVENTORY_PROJECTION_REFUSED' }
  foreach ($cursor in @($Value.pagination.next,$Value.pagination.prev)) {
    if ($null -ne $cursor -and (($cursor -isnot [int] -and $cursor -isnot [long]) -or $cursor -lt 0 -or ([string]$cursor).Length -gt 16)) { throw 'INVENTORY_PROJECTION_REFUSED' }
  }
  $rows = New-Object System.Collections.Generic.List[object]
  $hosts = New-Object System.Collections.Generic.HashSet[string]([StringComparer]::Ordinal)
  foreach ($row in @($Value.rows)) {
    Assert-ExactJsonObject -Value $row -AllowedFields @('deploymentHost','stateClass','targetClass','createdAt') -RequiredFields @('deploymentHost','stateClass','targetClass','createdAt') -RefusalCode 'INVENTORY_PROJECTION_REFUSED'
    if (-not (Test-StrictDeploymentHostname $row.deploymentHost) -or -not $hosts.Add($row.deploymentHost) -or
      $row.stateClass -isnot [string] -or $row.stateClass -cnotin @('READY','ERROR','CANCELED','BUILDING','INITIALIZING','QUEUED') -or
      $row.targetClass -isnot [string] -or $row.targetClass -cnotin @('production','preview') -or
      ($row.createdAt -isnot [int] -and $row.createdAt -isnot [long]) -or $row.createdAt -lt 0) { throw 'INVENTORY_PROJECTION_REFUSED' }
    $rows.Add([pscustomobject]@{DeploymentHost=$row.deploymentHost;StateClass=$row.stateClass;TargetClass=$row.targetClass;CreatedAt=[long]$row.createdAt})
  }
  return [pscustomobject]@{Rows=$rows.ToArray();Pagination=[pscustomobject]@{Count=[int]$Value.pagination.count;Next=$Value.pagination.next;Prev=$Value.pagination.prev}}
}

function Invoke-InventoryProjectorPage {
  param([bool]$OwnedOnly, [AllowNull()][object]$Next = $null)
  $projector = Join-Path $canonicalWorkspace 'scripts\vercel-deployment-inventory-projection-029W.mjs'
  if (-not (Test-Path -LiteralPath $projector) -or (Get-FileHash -LiteralPath $projector -Algorithm SHA256).Hash.ToLowerInvariant() -cne $inventoryProjectorSha) { throw 'INVENTORY_PROJECTOR_SOURCE_REFUSED' }
  $node = Get-Command node.exe -ErrorAction Stop
  $args = @('scripts/vercel-deployment-inventory-projection-029W.mjs','--mode',$(if($OwnedOnly){'owned'}else{'full'}))
  if ($null -ne $Next) {
    if (($Next -isnot [int] -and $Next -isnot [long]) -or $Next -lt 0 -or ([string]$Next) -notmatch '^[0-9]{1,16}$') { throw 'INVENTORY_PROJECTOR_VECTOR_REFUSED' }
    $args += @('--next',[string]$Next)
  }
  $result = Invoke-CapturedChildProcess -ExecutablePath $node.Source -CommandArgs $args -InputWasBound $false -InputValue $null -VectorClass 'inventory-projector'
  if ($null -eq $result -or $result.ExitCode -ne 0 -or $result.VectorClass -cne 'inventory-projector' -or
    $result.Stdout -isnot [string] -or -not $result.Stdout -or $result.Stdout.Length -gt 1048576 -or
    $result.Stderr -isnot [string] -or $result.Stderr) { throw 'INVENTORY_PROJECTOR_REFUSED' }
  try { $value = $result.Stdout | ConvertFrom-Json } catch { throw 'INVENTORY_PROJECTOR_REFUSED' }
  return ConvertTo-DeploymentInventoryProjection -Value $value
}

function Get-DeploymentPage {
  param([bool]$OwnedOnly, [AllowNull()][object]$Next = $null)
  return Invoke-InventoryProjectorPage -OwnedOnly $OwnedOnly -Next $Next
}
...
function Invoke-DeploymentWalk {
  param(
    [bool]$OwnedOnly,
    [scriptblock]$Resolver = { param([bool]$IsOwned,[AllowNull()][object]$Cursor) Get-DeploymentPage -OwnedOnly $IsOwned -Next $Cursor }
  )
  $first = & $Resolver $OwnedOnly $null
  $all = New-Object System.Collections.Generic.List[object]
  $seen = New-Object System.Collections.Generic.HashSet[string]([StringComparer]::Ordinal)
  $cursors = New-Object System.Collections.Generic.HashSet[string]([StringComparer]::Ordinal)
  foreach ($row in $first.Rows) { if (-not $seen.Add($row.DeploymentHost)) { throw 'DEPLOYMENT_SNAPSHOT_REFUSED' }; $all.Add($row) }
  $pages = 1
  $next = $first.Pagination.Next
  while ($null -ne $next) {
    if ($pages -ge 10 -or $all.Count -ge 200 -or -not $cursors.Add([string]$next)) { throw 'DEPLOYMENT_SNAPSHOT_REFUSED' }
    $page = & $Resolver $OwnedOnly $next
    $pages += 1
    foreach ($row in $page.Rows) {
      if (-not $seen.Add($row.DeploymentHost) -or $all.Count -ge 200) { throw 'DEPLOYMENT_SNAPSHOT_REFUSED' }
      $all.Add($row)
    }
    $next = $page.Pagination.Next
  }
  $head = & $Resolver $OwnedOnly $null
  return [pscustomobject]@{Rows=$all.ToArray();Pages=$pages;FirstHead=$first;RevalidatedHead=$head;HeadStable=(Test-DeploymentHeadEqual $first $head)}
}
function Get-StableDeploymentInventory {
  ...
  for ($attempt=0; $attempt -lt 2; $attempt += 1) {
    $walk = Invoke-DeploymentWalk -OwnedOnly $OwnedOnly -Resolver $Resolver
    if ($walk.HeadStable) { return [pscustomobject]@{Rows=@($walk.Rows);Pages=$walk.Pages;RestartCount=$attempt;HeadStable=$true} }
  }
  throw 'DEPLOYMENT_SNAPSHOT_REFUSED'
}function Get-StableFullDeploymentInventory { return Get-StableDeploymentInventory -OwnedOnly $false }
function Get-StableOwnedDeploymentInventory { return Get-StableDeploymentInventory -OwnedOnly $true }
function Get-DeploymentListProjection {
  $inventory = Get-StableOwnedDeploymentInventory
  $ids = New-Object System.Collections.Generic.List[string]
  foreach ($row in $inventory.Rows) {
    $projection = Get-OwnedDeploymentProjectionByHost -DeploymentHost $row.DeploymentHost
    $ids.Add($projection.DeploymentId)
  }
  return $ids.ToArray()
}
...
function ConvertTo-DeploymentProjection {
  ...
  Assert-ExactJsonObject -Value $Value -AllowedFields @('id','name','url','target','readyState','createdAt','aliases','builds','routes','contextName','meta','sourceSha') -RequiredFields @('id','name','url','target','readyState','aliases') -RefusalCode 'DEPLOYMENT_REFUSED'
  $id = $Value.id
  $targetClass = Get-KnownDeploymentTargetClass -Target $Value.target
  $ready = $Value.readyState
  if ($null -eq $value.aliases -or $value.aliases -isnot [array]) { throw 'DEPLOYMENT_REFUSED' }
  ...
  return [pscustomobject]@{
    ...
    MetadataClass=$(if($meta -and $meta.PSObject.Properties.Name -ccontains 'pp_sprint' -and $meta.pp_sprint -ceq '029W'){'exact-029W'}else{'historical'})
    SourceClass=$(if($sourceSha -ceq $expectedHead -or $DeploymentId -cin $fixedInspectIds){'exact-canonical'}else{'refused'})
  }
}

function Get-OwnedDeploymentProjectionByHost {
  param(
    [Parameter(Mandatory)][string]$DeploymentHost,
    [scriptblock]$Resolver = { param([string]$DeploymentReference) Invoke-VercelCaptured -CommandArgs @('inspect',$DeploymentReference,'--format','json','--no-color') }
  )
  if (-not (Test-StrictDeploymentHostname $DeploymentHost)) { throw 'DEPLOYMENT_HOST_REFUSED' }
  $script:OwnedDeploymentHost = $DeploymentHost
  try {
    $result = & $Resolver $DeploymentHost
    if ($result.Json.id -isnot [string] -or $result.Json.id -notmatch '^dpl_[A-Za-z0-9]+$') { throw 'DEPLOYMENT_OWNERSHIP_REFUSED' }
    $projection = ConvertTo-DeploymentProjection -Value $result.Json -DeploymentId $result.Json.id
    if (([uri]$projection.Origin).Host -cne $DeploymentHost -or $projection.TargetClass -ne 'production' -or
      $projection.ReadinessClass -ne 'ready' -or $projection.AliasCount -ne 0 -or
      $projection.MetadataClass -ne 'exact-029W' -or $projection.SourceClass -ne 'exact-canonical') { throw 'DEPLOYMENT_OWNERSHIP_REFUSED' }
    $script:OwnedDeploymentId = $projection.DeploymentId
    return $projection
  } finally { $script:OwnedDeploymentHost = '' }
}
...
function Assert-ExactOwnedCandidate {
  ...
  if ($candidate.TargetClass -ne 'production' -or $candidate.ReadinessClass -ne 'ready' -or
    $candidate.AliasCount -ne 0 -or $candidate.MetadataClass -ne 'exact-029W' -or
...
function Invoke-DeployWithReconciliation {
  ...
  $directFailure = $null
  try {
    $deployed = Invoke-VercelCaptured -CommandArgs @('deploy','--prod','--skip-domain','--yes','--format','json','--no-color','--meta','pp_sprint=029W')
  ...
  $preIds = @($PreInventory.Rows | ForEach-Object { $_.DeploymentHost })
  $postIds = @($postInventory.Rows | ForEach-Object { $_.DeploymentHost })
  $added = @($postIds | Where-Object { $_ -cnotin $preIds })
  $removed = @($preIds | Where-Object { $_ -cnotin $postIds })
  if ($added.Count -ne 1 -or $added[0] -cne $direct.Host -or $removed.Count -ne 0) { throw 'DEPLOYMENT_COMPETING_MATERIAL' }
...
function Assert-BaselineProjection {
  ...
  if ($SprintDeploymentCount -ne 0) { Stop-BaselineFailure 'BASELINE_029W_DEPLOYMENT_REFUSED' }
...
function Invoke-ControllerSelfTest {
  ...
  $allowed.Add(@{ Args=@('deploy','--prod','--skip-domain','--yes','--format','json','--no-color','--meta','pp_sprint=029W'); Input=$false })
  $script:OwnedDeploymentId = 'dpl_Synthetic029W'
  $allowed.Add(@{ Args=@('remove','dpl_Synthetic029W','--safe','--yes','--no-color'); Input=$false })
...
$inventoryProjectorSha = 'ad04b8711775ecbedd0acd36102455b9745bc52490a9f480efc0e02f3b5af142'
  ...
  $processTransfer = Invoke-CapturedChildProcess -ExecutablePath $env:ComSpec -CommandArgs @('/d','/s','/c','echo','PP029W_ONE','PP029W_TWO','PP029W_THREE','PP029W_FOUR') -InputWasBound $false -InputValue $null -VectorClass 'self-test-process'
  if ($processTransfer.ExitCode -ne 0 -or $processTransfer.VectorClass -cne 'self-test-process' -or
    $processTransfer.Stdout.Trim() -cne 'PP029W_ONE PP029W_TWO PP029W_THREE PP029W_FOUR' -or $processTransfer.Stderr) { throw 'SELF_TEST_REFUSED' }
  ...
  $emptyDeploymentProjection = ConvertTo-DeploymentInventoryProjection -Value ([pscustomobject]@{projectionVersion=1;rows=@();pagination=[pscustomobject]@{count=0;next=$null;prev=$null}})
  if ($emptyDeploymentProjection.Rows.Count -ne 0 -or $emptyDeploymentProjection.Pagination.Count -ne 0) { throw 'SELF_TEST_REFUSED' }
  ...
  $rawDeployFixture = [pscustomobject]@{
    id='dpl_Synthetic029W'
    url='https://pnr-precision-performance-synthetic-rankin007s-projects.vercel.app'
    inspectorUrl=$null
    readyState='READY'
    target='production'
    deploymentApiUrl='https://api.vercel.com/v13/deployments/dpl_Synthetic029W'
  }
  ...
  $agentEnvelopeFixturePassCount = if($agentProjection.DeploymentId -ceq 'dpl_Synthetic029W' -and $agentProjection.Form -ceq 'agent'){1}else{0}
  $agentEnvelopeRefusalCount = 0
  $agentRefusals = @(
    '{"status":"OK","deployment":{"id":"dpl_Synthetic029W","url":"https://pnr-precision-performance-synthetic-rankin007s-projects.vercel.app","inspectorUrl":null,"readyState":"READY","target":"production","deploymentApiUrl":"https://api.vercel.com/v13/deployments/dpl_Synthetic029W"},"message":"Deployment pnr-precision-performance-synthetic-rankin007s-projects.vercel.app ready.","next":[{"command":"vercel inspect pnr-precision-performance-synthetic-rankin007s-projects.vercel.app --no-color","when":"Inspect deployment"},{"command":"vercel deploy --prod --no-color","when":"Promote to production"}]}',
    '{"status":"ok","deployment":{"id":"dpl_Synthetic029W","url":"https://pnr-precision-performance-synthetic-rankin007s-projects.vercel.app","inspectorUrl":null,"readyState":"READY","target":"production","deploymentApiUrl":"https://api.vercel.com/v13/deployments/dpl_Synthetic029W"},"message":"Deployment https://pnr-precision-performance-synthetic-rankin007s-projects.vercel.app ready.","next":[{"command":"vercel inspect pnr-precision-performance-synthetic-rankin007s-projects.vercel.app --no-color","when":"Inspect deployment"},{"command":"vercel deploy --prod --no-color","when":"Promote to production"}]}',
    ...
    '{"status":"ok","deployment":{"id":"dpl_Synthetic029W","url":"https://pnr-precision-performance-synthetic-rankin007s-projects.vercel.app","inspectorUrl":null,"readyState":"READY","target":"production","deploymentApiUrl":"https://api.vercel.com/v13/deployments/dpl_Synthetic029W","extra":0},"message":"Deployment pnr-precision-performance-synthetic-rankin007s-projects.vercel.app ready.","next":[{"command":"vercel inspect pnr-precision-performance-synthetic-rankin007s-projects.vercel.app --no-color","when":"Inspect deployment"},{"command":"vercel deploy --prod --no-color","when":"Promote to production"}]}',
  ...
  $projectionFixture = ConvertTo-DeploymentInventoryProjection -Value ([pscustomobject]@{
    projectionVersion=1
    rows=@([pscustomobject]@{deploymentHost='synthetic.vercel.app';stateClass='READY';targetClass='production';createdAt=1})
    pagination=[pscustomobject]@{count=1;next=1234567890;prev=$null}
  })
  if ($projectionFixture.Pagination.Next -eq 1234567890 -and $projectionFixture.Rows[0].DeploymentHost -ceq 'synthetic.vercel.app') { $paginationFixturePassCount = 1 }
  $paginationRefusalCount = 0
  foreach ($fixture in @(
    [pscustomobject]@{projectionVersion=2;rows=@();pagination=[pscustomobject]@{count=0;next=$null;prev=$null}},
    [pscustomobject]@{projectionVersion=1;rows=@();pagination=[pscustomobject]@{count=1;next=$null;prev=$null}}
  )) {
    try { [void](ConvertTo-DeploymentInventoryProjection -Value $fixture) } catch {
      if ($_.Exception.Message -cin @('INVENTORY_PROJECTION_REFUSED','INVENTORY_PROJECTION_VERSION_REFUSED')) { $paginationRefusalCount += 1 } else { throw }
    }
  }
  if ($paginationFixturePassCount -ne 1 -or $paginationRefusalCount -ne 2) { throw 'SELF_TEST_REFUSED' }
  $protectedInventoryRefusalCount = 2

  $headDriftNullCalls = [pscustomobject]@{Count=0}
  $headDriftResolver = {
    param([bool]$IsOwned,[AllowNull()][object]$Cursor)
    if ($null -eq $Cursor) {
      $headDriftNullCalls.Count += 1
      $nextValue = if (($headDriftNullCalls.Count % 2) -eq 1) { 1 } else { 2 }
      return [pscustomobject]@{Rows=@([pscustomobject]@{DeploymentHost='head.vercel.app';StateClass='READY';TargetClass='production';CreatedAt=1});Pagination=[pscustomobject]@{Count=1;Next=$nextValue;Prev=$null}}
    }
    return [pscustomobject]@{Rows=@([pscustomobject]@{DeploymentHost='tail.vercel.app';StateClass='READY';TargetClass='production';CreatedAt=2});Pagination=[pscustomobject]@{Count=1;Next=$null;Prev=1}}
  }.GetNewClosure()
  $headPaginationDriftRefusalCount = 0
  try { [void](Get-StableDeploymentInventory -OwnedOnly $false -Resolver $headDriftResolver) } catch {
    if ($_.Exception.Message -ceq 'DEPLOYMENT_SNAPSHOT_REFUSED') { $headPaginationDriftRefusalCount = 1 } else { throw }
  }
  if ($headPaginationDriftRefusalCount -ne 1 -or $headDriftNullCalls.Count -ne 4) { throw 'SELF_TEST_REFUSED' }

  $restartNullCalls = [pscustomobject]@{Count=0}
  $restartResolver = {
    param([bool]$IsOwned,[AllowNull()][object]$Cursor)
    if ($null -eq $Cursor) {
      $restartNullCalls.Count += 1
      $nextValue = if ($restartNullCalls.Count -eq 2) { 2 } else { 1 }
      return [pscustomobject]@{Rows=@([pscustomobject]@{DeploymentHost='head.vercel.app';StateClass='READY';TargetClass='production';CreatedAt=1});Pagination=[pscustomobject]@{Count=1;Next=$nextValue;Prev=$null}}
    }
    return [pscustomobject]@{Rows=@([pscustomobject]@{DeploymentHost='tail.vercel.app';StateClass='READY';TargetClass='production';CreatedAt=2});Pagination=[pscustomobject]@{Count=1;Next=$null;Prev=1}}
  }.GetNewClosure()
  $restartProjection = Get-StableDeploymentInventory -OwnedOnly $false -Resolver $restartResolver
  $headRestartPassCount = if ($restartProjection.RestartCount -eq 1 -and $restartProjection.HeadStable -and $restartNullCalls.Count -eq 4) { 1 } else { 0 }
  if ($headRestartPassCount -ne 1) { throw 'SELF_TEST_REFUSED' }

  $walkCeilingRefusalCount = 0
  $cursorLoopResolver = {
    param([bool]$IsOwned,[AllowNull()][object]$Cursor)
    $fixtureHost = if ($null -eq $Cursor) { 'first.vercel.app' } else { 'second.vercel.app' }
    return [pscustomobject]@{Rows=@([pscustomobject]@{DeploymentHost=$fixtureHost;StateClass='READY';TargetClass='production';CreatedAt=1});Pagination=[pscustomobject]@{Count=1;Next=1;Prev=$null}}
  }
  try { [void](Invoke-DeploymentWalk -OwnedOnly $false -Resolver $cursorLoopResolver) } catch { if ($_.Exception.Message -ceq 'DEPLOYMENT_SNAPSHOT_REFUSED') { $walkCeilingRefusalCount += 1 } else { throw } }
  $pageCeilingResolver = {
    param([bool]$IsOwned,[AllowNull()][object]$Cursor)
    $number = if ($null -eq $Cursor) { 0 } else { [int]$Cursor }
    return [pscustomobject]@{Rows=@([pscustomobject]@{DeploymentHost=('page' + $number + '.vercel.app');StateClass='READY';TargetClass='production';CreatedAt=$number});Pagination=[pscustomobject]@{Count=1;Next=($number+1);Prev=$null}}
  }
  try { [void](Invoke-DeploymentWalk -OwnedOnly $false -Resolver $pageCeilingResolver) } catch { if ($_.Exception.Message -ceq 'DEPLOYMENT_SNAPSHOT_REFUSED') { $walkCeilingRefusalCount += 1 } else { throw } }
  $rowCeilingResolver = {
    param([bool]$IsOwned,[AllowNull()][object]$Cursor)
    $rows = @(0..199 | ForEach-Object { [pscustomobject]@{DeploymentHost=('row' + $_ + '.vercel.app');StateClass='READY';TargetClass='production';CreatedAt=$_} })
    return [pscustomobject]@{Rows=$rows;Pagination=[pscustomobject]@{Count=200;Next=1;Prev=$null}}
  }
  try { [void](Invoke-DeploymentWalk -OwnedOnly $false -Resolver $rowCeilingResolver) } catch { if ($_.Exception.Message -ceq 'DEPLOYMENT_SNAPSHOT_REFUSED') { $walkCeilingRefusalCount += 1 } else { throw } }
  if ($walkCeilingRefusalCount -ne 3) { throw 'SELF_TEST_REFUSED' }

  $activeStateFixtureCount = Get-ActiveDeploymentCount @(
    [pscustomobject]@{StateClass='BUILDING'},
    [pscustomobject]@{StateClass='INITIALIZING'},
    [pscustomobject]@{StateClass='QUEUED'},
    [pscustomobject]@{StateClass='READY'}
  )
  if ($activeStateFixtureCount -ne 3) { throw 'SELF_TEST_REFUSED' }

  $ownedHostFixture = 'pnr-precision-performance-owned029w-rankin007s-projects.vercel.app'
  $makeOwnedInspectFixture = {
    [pscustomobject]@{
      id='dpl_Owned029W';name='synthetic';url=$ownedHostFixture;target='production';readyState='READY';aliases=@();
      meta=[pscustomobject]@{pp_sprint='029W'};sourceSha=$expectedHead
    }
  }.GetNewClosure()
  $goodOwnedFixture = & $makeOwnedInspectFixture
  $goodOwnedResolver = { param([string]$DeploymentReference) [pscustomobject]@{VectorClass='inspect-owned-host';Json=$goodOwnedFixture} }.GetNewClosure()
  $goodOwnedProjection = Get-OwnedDeploymentProjectionByHost -DeploymentHost $ownedHostFixture -Resolver $goodOwnedResolver
  $ownershipFixturePassCount = if ($goodOwnedProjection.DeploymentId -ceq 'dpl_Owned029W' -and $goodOwnedProjection.AliasCount -eq 0) { 1 } else { 0 }
  $script:OwnedDeploymentId = ''
  $ownershipMismatchRefusalCount = 0
  $ownershipCases = @(
    { $v=& $makeOwnedInspectFixture; $v.PSObject.Properties.Remove('aliases'); $v },
    { $v=& $makeOwnedInspectFixture; $v.aliases=$null; $v },
    { $v=& $makeOwnedInspectFixture; $v.aliases='not-array'; $v },
    { $v=& $makeOwnedInspectFixture; $v.aliases=@('unexpected.vercel.app'); $v },
    { $v=& $makeOwnedInspectFixture; $v.id='bad-id'; $v },
    { $v=& $makeOwnedInspectFixture; $v.url='pnr-precision-performance-different-rankin007s-projects.vercel.app'; $v },
    { $v=& $makeOwnedInspectFixture; $v.target='preview'; $v },
    { $v=& $makeOwnedInspectFixture; $v.readyState='ERROR'; $v },
    { $v=& $makeOwnedInspectFixture; $v.meta.pp_sprint='other'; $v },
    { $v=& $makeOwnedInspectFixture; $v.sourceSha='0000000000000000000000000000000000000000'; $v }
  )
  foreach ($case in $ownershipCases) {
    $inspectValue = & $case
    $badOwnedResolver = { param([string]$DeploymentReference) [pscustomobject]@{VectorClass='inspect-owned-host';Json=$inspectValue} }.GetNewClosure()
    try { [void](Get-OwnedDeploymentProjectionByHost -DeploymentHost $ownedHostFixture -Resolver $badOwnedResolver) } catch {
      if ($_.Exception.Message -cin @('DEPLOYMENT_REFUSED','DEPLOYMENT_OWNERSHIP_REFUSED')) { $ownershipMismatchRefusalCount += 1 } else { throw }
    }
  }
  if ($ownershipFixturePassCount -ne 1 -or $ownershipMismatchRefusalCount -ne 10 -or $script:OwnedDeploymentHost) { throw 'SELF_TEST_REFUSED' }
  ...
  $syntheticRecord = Complete-LivePhase -Record $syntheticRecord -ExpectedPhase 'deploy-attempted' -CompletedPhase 'deployed' -DeploymentId 'dpl_SyntheticPhase029W' -Writer $phaseWriter
  ...
  $originRecord = [pscustomobject]@{
    ...
    remoteNames=@($temporaryNames);deploymentId='dpl_SyntheticOrigin029W';phase='deployed';deployAttemptCount=1;publicGateAttemptCount=0;readinessAttemptCount=0;expiryAttemptCount=0
  }
  $acceptedOriginFixture = 'https://pnr-precision-performance-accepted029w-rankin007s-projects.vercel.app'
  $wrongOriginFixture = 'https://pnr-precision-performance-wrong029w-rankin007s-projects.vercel.app'
  $originResolver = { param([string]$Id) if($Id -cne 'dpl_SyntheticOrigin029W'){throw 'DEPLOYMENT_REFUSED'}; [pscustomobject]@{Origin=$acceptedOriginFixture} }.GetNewClosure()
  ...
  foreach ($shapeCase in @(
    ...
    @{ Code='INVENTORY_PROJECTION_REFUSED'; Run={ [void](ConvertTo-DeploymentInventoryProjection -Value ([pscustomobject]@{ projectionVersion=1; rows=@([pscustomobject]@{ deploymentHost='synthetic.vercel.app'; stateClass='READY'; targetClass='production'; createdAt=1; extra='refuse' }); pagination=[pscustomobject]@{count=1;next=$null;prev=$null} })) } },
  ...
  foreach ($targetCase in @(
    ...
    { [void](ConvertTo-DeploymentInventoryProjection -Value ([pscustomobject]@{ projectionVersion=1; rows=@([pscustomobject]@{ deploymentHost='synthetic.vercel.app'; stateClass='READY'; targetClass='staging'; createdAt=1 }); pagination=[pscustomobject]@{count=1;next=$null;prev=$null} })) },
    ...
    $didRefuse = $false
    try { & $targetCase } catch { if ($_.Exception.Message -cin @('DEPLOYMENT_REFUSED','INVENTORY_PROJECTION_REFUSED')) { $didRefuse = $true } else { throw } }
    if (-not $didRefuse) { throw 'SELF_TEST_REFUSED' }
  ...
  foreach ($failureCode in $baselineFailureCodes) {
    ...
    switch -CaseSensitive ($failureCode) {
      ...
      'BASELINE_029W_DEPLOYMENT_REFUSED' { $deploymentCountFixture = 1 }
  ...
  $refused = @(
    ...
    @('deploy','--prod','--skip-domain','--yes','--format','json','--no-color','--meta','pp_sprint=029W','--force'),
    @('deploy','--prod','--yes','--format','json','--no-color','--meta','pp_sprint=029W'),
...
if ($Operation -eq 'CredentialSelfTest') {
  ...
  '{"controller":"029W","operation":"credential-self-test","state":"pass","credentialResidue":0}'
  exit 0
}
...
if ($Operation -eq 'Baseline') {
  ...
  try { $sprintDeployments = Get-DeploymentListProjection } catch { Stop-BaselineFailure 'BASELINE_029W_DEPLOYMENT_REFUSED' }
  ...
  [pscustomobject]@{controller='029W';operation='baseline';state='pass';projectClass='exact';dedicatedSmtpRowCount=0;temporaryAuthRowCount=0;activationRowCount=0;genericSmtpRowCount=5;credentialState='absent';sprintDeploymentCount=0;retainedPreviewClass='ready-preview-inert';retainedPreviewAliasCount=0;priorCandidateCount=$priorCandidateCount;priorCandidateAliasCount=0;acceptedDeploymentClass='ready';acceptedAliasTargetCount=5} | ConvertTo-Json -Compress
  exit 0
}
...
if ($Operation -eq 'AcceptPrivatePasswordBaseline') {
  ...
  '{"controller":"029W","operation":"accept-private-password-baseline","state":"pass","dedicatedSmtpRowCount":1,"temporaryAuthRowCount":0,"activationRowCount":0,"genericSmtpRowCount":5,"passwordSensitivityClass":"exact-sensitive-production","sprintDeploymentCount":0,"acceptedAliasTargetCount":5}'
  exit 0
}
...
if ($Operation -eq 'AddStructuralSmtp') {
  ...
  '{"controller":"029W","operation":"add-structural-smtp","state":"pass","dedicatedSmtpRowCount":4,"temporaryAuthRowCount":0,"activationRowCount":0,"sensitivityClass":"all-exact-sensitive-production"}'
  exit 0
}
...
if ($Operation -eq 'Provision') {
  ...
  try {
    ...
    '{"controller":"029W","operation":"provision","state":"pass","bindingCount":3,"windowClass":"bounded","credentialState":"present","activationRowCount":0,"sensitivityClass":"all-seven-exact-sensitive-production"}'
  } catch {
    $operationFailure = $_.Exception.Message
...
if ($Operation -eq 'Status') {
  ...
  if ($deploymentIds.Count -eq 1) {
    ...
    if ($candidate.TargetClass -ne 'production' -or $candidate.MetadataClass -ne 'exact-029W') { throw 'DEPLOYMENT_REFUSED' }
...
if ($Operation -eq 'Compensate') {
  ...
  try {
    ...
    '{"controller":"029W","operation":"compensate","state":"pass","bindingResidue":0,"credentialResidue":0,"deploymentResidue":0,"acceptedAliasTargetCount":5}'
  } finally { if($record){$record.bearer=$null;$record.verifier=$null};$record=$null }
  exit 0
}
...
try {
  ...
  if ($Operation -eq 'VerifyPublicGate') {
    ...
    '{"controller":"029W","operation":"verify-public-gate","state":"pass","requestCount":1,"httpClass":"service-unavailable","responseClass":"sanitized","productActionCount":0}'
    ...
    '{"controller":"029W","operation":"verify-ready","state":"pass","requestCount":1,"providerClass":"resend","errorClass":null}'
    ...
    '{"controller":"029W","operation":"expired-denial-cleanup","state":"pass","requestCount":1,"httpClass":"not-found","bindingResidue":0,"credentialResidue":0}'
  }
} finally {
