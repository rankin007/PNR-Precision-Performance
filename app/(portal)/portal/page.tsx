...
export default async function PortalPage({ searchParams }: PortalPageProps) {
  ...
  return (
    <SectionCard
      ...
      title="Your assigned horses"
      description="Open an assigned horse to review current operational context and continue an available biochemistry task. Information shown here supports trainer review and is not clinical advice."
    >
      {denied === "admin" ? (
      ...
      <div className="mb-6 rounded-2xl border border-ink/10 bg-sand px-4 py-4 text-sm text-steel" role="status">
        Assigned horses are listed alphabetically. Workflow labels describe record state only; clinical priority is unavailable.
      </div>
      {"error" in overview && overview.error ? (
        <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-900" role="alert">
          Assigned-horse information is unavailable right now. No missing information has been treated as complete or normal.
        </div>
      ) : null}
      <div className="grid gap-4" aria-label="Assigned horse operational overview">
        ...
        {overview.horses.length === 0 && !("error" in overview && overview.error) ? <p className="rounded-2xl border border-ink/10 bg-white p-5 text-sm text-steel">No horses are currently assigned to this account. Contact the authorised access owner if an assignment is expected.</p> : null}
      </div>
    </SectionCard>
  );
}
