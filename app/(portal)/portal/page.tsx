...
export default async function PortalPage({ searchParams }: PortalPageProps) {
  ...
  return (
    <SectionCard
      ...
      title="Assigned horse dashboard"
      description="Review the horses assigned to this approved account and continue an existing operational workflow."
    >
      {denied === "admin" ? (
      ...
      <div className="mb-6 rounded-2xl border border-ink/10 bg-sand px-4 py-4 text-sm text-steel" role="status">
        <strong className="text-ink">{overview.presentation.title}.</strong> {overview.presentation.message} Operational attention identifies incomplete workflow records only; clinical priority is unavailable.
      </div>
      <div className="grid gap-4" aria-label="Assigned horse operational overview">
        ...
        {overview.horses.length === 0 ? <p className="rounded-2xl border border-ink/10 bg-white p-5 text-sm text-steel">{overview.presentation.message}</p> : null}
      </div>
    </SectionCard>
  );
}
