...
import { initiateUpload, lifecycleMutation, listEvidence, requestDownload, resolveEvidenceActor } from "@/lib/evidence/server/repository";
...
export async function replaceEvidence(testId: string, uploadId: string) { const actor = await actorFor(testId); return actor ? lifecycleMutation(actor, "replace", uploadId) : { ok: false as const, code: "denied" as const }; }
...
export async function purgeEvidence(testId: string, uploadId: string) { const actor = await actorFor(testId); return actor ? lifecycleMutation(actor, "purge", uploadId) : { ok: false as const, code: "denied" as const }; }
