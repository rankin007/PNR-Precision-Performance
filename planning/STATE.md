...
Sprint `029I-live-public-content-reconciliation-and-hard-deploy` is closed as `marketing-preview-live-reconciled`. The Sprint 029 root marketing-preview page is live at `https://precisionperformance.com.au`, and 029B corrected the suspect front-page width/layout classes in `app/page.tsx`.

Sprint 029F deployed the browser-proof marketing-preview release. Sprint 029G reconciled the production alias after a reported discrepancy. Sprint 029H added stronger external-public verification. Sprint 029I reconciled source, clean-worktree build, Vercel alias/deployment, DNS/header, and live public content evidence after a later old-content observation. Cache-busted live requests to apex, `www`, the Vercel app alias, and the exact reported URL `https://precisionperformance.com.au/?review029h=1` returned Sprint 029 markers and no old-page markers; DNS resolved apex and `www` to Vercel records; Vercel inspect maps the aliases to deployment `dpl_9gPytpAofTSHcTJJMM1Qw9TxKpAd`; and public route-safety smoke passes. The raw deployment URL is Vercel SSO-gated. No alias correction, redeploy, DNS change, Vercel setting/environment mutation, Supabase mutation, Stripe mutation, or production data mutation occurred in 029I. Unresolved active Sprint 021AA dirty-tree state remains outside the 029I scope.
...
Immediate Sprint 029 follow-up: preserve the marketing-preview deployment boundary and plan any broader public website, commerce, authenticated, SEO/indexing, or production-readiness work separately. If an operator or reviewer still sees old public content, first hard-refresh/private-window test the exact live URL and a cache-busted URL because 029I live HTTP checks of the reported `?review029h=1` URL returned current Sprint 029 content rather than old alias behavior.
