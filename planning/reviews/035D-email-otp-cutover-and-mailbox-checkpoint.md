...
## Manual-preview incident reconciliation and cleanup

The operator confirmed the bounded manual trainer preview was complete. Guarded enumeration in approved project `uvskssaecdhxcgytkasc` found four identities: one Participant A Sprint-owned identity, one exact manual-preview synthetic identity and two identities preserved as ambiguous. An operator-authorized hidden exact-match cleanup returned zero matches; no identity or application mutation occurred and the operator-created incident identity remains `preserved-ambiguous`. A second hidden-input attempt was not permitted.

The exact manual Preview-origin browser session was signed out without clearing Gmail, the Chrome profile or unrelated site state. A reload of that exact Preview origin returned to the public root without a query artifact. Initial page inspection unexpectedly emitted protected application text beyond the requested boolean boundary; those values were not reproduced, retained, correlated or used for deletion. Cleanup selection used only the administrator-controlled manual-preview run marker and its exact foreign-key graph.

The exact graph contained eight application/profile/access/fixture records, one Auth identity and zero owned Storage objects. Cleanup removed the eight records dependency-safely, deleted the exact run-owned Auth identity last and proved its absence. Final manual-preview application/Auth/Storage is `0/0/0`; total Auth count is three; Participant A remains preserved; both ambiguous identities remain unchanged. Parent service-role environment is absent. No email, OTP request, participant authentication or provider mutation occurred.

Read-only Vercel alias inventory resolved the apparent four-of-five failure. The fifth accepted alias, `pnr-precision-performance-rankin007-rankin007s-projects.vercel.app`, remains attached to corrected production and was omitted only by the deployment-inspection view. All five accepted aliases map to `dpl_9zVS8HSujThkFTP3hisyfBVknKWb`; apex and `www` independently return `200`. Exact manual Preview `dpl_78mjFb5vS6kvWeGWsX1wT1uZhnig` remains Ready/Preview and assigned aliases are zero. Forward rollback `dpl_EUJyBLQp7okgbPwtBU6nmMH88L6A` remains Ready.

Protected provider readback confirms the unchanged production Site URL, exactly two callbacks (production plus corrected alias-free Preview), and no manual-Preview callback. The current OTP template canonical fingerprint remains `23c6a254d6c68cff676c99500768c1ef8eb7d33ff85eb55c0e409701bd330b49`, with one `.Token`, zero `ConfirmationURL` and zero links. GitHub comparison proves local and remote scoped branch tips identical at `494d0db598a451cebf0aa985fe5f9f602cfdb513`; the earlier difference was an SSH/upstream-observation failure, not divergence.

## Final guarded safety check and closeout

Final read-only enumeration proves exactly three Auth identities remain in approved project `uvskssaecdhxcgytkasc`. Participant A remains tagged, Sprint-owned and unchanged. The two other identities remain preserved as ambiguous Auth-only identities: aggregate application/profile/access records `0`, owned Storage `0`, participant metadata `no`, active Sprint-owned session indicator `not-observed`, deletion authorization `absent`. No address was requested and no identity was deleted.

Sprint 035D closes `email-otp-validation-blocked-clean`. The OTP-capable application and OTP-only template are deployed and mutually compatible. Corrected production and the OTP-capable forward rollback are Ready. Five stable aliases, Site URL and the production/corrected-Preview callbacks pass. Exact-owned manual-preview and synthetic cleanup is `0/0/0`. Participant A and both ambiguous identities are preserved.

Delivered OTP authentication is not accepted and Participant A/B/C acceptance is not claimed. Supabase built-in SMTP is unsuitable for production or participant delivery because its recipient restrictions, low shared limits and best-effort delivery cannot satisfy the required delivery boundary. Custom transactional SMTP is required before any further synthetic or participant authentication. Product-wide Done is not claimed.
