...
### Fresh A Apply guard correction

A fresh A-only passwordless request was issued from the corrected exact Preview. The generated email remains unopened. Apply returned `ALIAS_ALREADY_PROCESSED_A` before hidden inbox entry and before Admin metadata mutation because the helper unconditionally rejected any prior alias entry, including the exact `contained-owned-deleted` state required for fresh ownership classification. Parent service-role environment is absent; protected ledger is unchanged; no participant metadata, application user/profile/membership/assignment or authenticated session exists. Current owned application/Auth/Storage is `0/1/0`; B/C remain untouched; Preview/callback/production configuration is unchanged.

The corrected transition permits an existing ledger entry only for A with ownership `sprint-owned`, state `contained-owned-deleted`, the recorded creation-window marker and a valid containment checkpoint. Every other existing state is refused before hidden input. After protected matching it requires exactly one fresh post-checkpoint identity, no session indicator, no application user, no participant metadata and no conflicting A/035C tag. The Admin metadata merge preserves unrelated fields; only after success is the prior ledger state atomically replaced with `existing-tagged`. Ledger persistence failure restores original metadata. Output remains sanitized.

### Tagged A authentication checkpoint

The corrected Apply passed with sanitized ownership `sprint-owned`; the protected ledger contains only A as `existing-tagged`/`sprint-owned`. The passwordless request confirmation occurred on the exact Preview sign-in route and reported sent with next `/portal`, but no authenticated session was established. Subsequent old-message landings were sanitized as production root, no visible code query, `otp_expired`; all browser windows were then closed. No protected artifact or complete URL was retained, B/C remain untouched, and no A acceptance is claimed.

Because an expired, consumed or superseded message cannot establish fresh redirect behavior, the next gate is a protected read-only A verification. It reports only exact A/035C tag cardinality, Sprint ownership, provider sign-in-indicator classification, aggregate application/access state and the existing zero-owned Storage classification. Exactly one tagged Sprint-owned A with no sign-in indicator and zero application state is eligible for one controlled fresh-message retry. Any ambiguity, indicator or unexpected state requires ownership-bounded containment instead.

Protected verification returned pass: tagged Auth `1`, ownership `sprint-owned`, sign-in indicator `not-observed`, application `0`, owned Storage `0`, owned counts `0/1/0`, decision `fresh-message-retry-eligible`. No protected identifiers or authentication artifacts were received.

Pre-retry provider verification reconfirmed deployment `dpl_CGtFBxpQakJHi3x41hzcgpvmDL3A` Ready, exact `ppSourceSha` `5b55aea39abcb45db06a42312d3ff9a7f293f3f9`, Preview environment and assigned alias count `0`. Supabase Auth URL configuration contains exactly the unchanged production callback and `https://pnr-precision-performance-ge5io3pa3-rankin007s-projects.vercel.app/auth/callback`; stale Preview and wildcard callbacks are absent and production Site URL is unchanged. The accepted production deployment/source remains Ready with the same five stable aliases. Forwarded Preview protocol/host structural tests pass. Artifact-free callback navigation remained on the exact Preview and resolved through the intended `/portal` boundary to `/sign-in?next=%2Fportal` while unauthenticated. A is eligible for exactly one fresh-message retry; B/C remain untouched.

### Dual-action failure and root cause

The controlled retry did not authenticate. Both displayed actions produced only the sanitized result production host, root path, no authenticated session, `otp_expired`. Participant execution stopped; no protected URL or artifact was retained and B/C remain untouched.

Read-only provider inspection establishes that the active Magic Link template contains exactly one `ConfirmationURL` action labelled Sign in, while the separate Confirm sign up template contains exactly one `ConfirmationURL` action labelled Confirm email address. Email confirmation and new-user signup are enabled. Source inspection establishes that portal authentication used `signInWithOtp` with explicit Preview `emailRedirectTo` but omitted `shouldCreateUser`, whose Supabase default is true. The dual presentation is therefore the combination of signup/confirmation and subsequent sign-in material, not one provider template generating two actions. Supabase's official passwordless/template guidance also documents that mail security prefetch can consume `ConfirmationURL` before the participant opens it and produce expired/invalid-token behavior; this explains failure of the first manually opened action, while opening the second or a superseded action can independently fail. Once verification is invalid, the observed production-root error is consistent with provider Site URL fallback and does not establish a valid Preview redirect.

The in-scope correction sets `shouldCreateUser: false`, so the controlled participant portal cannot create an identity or trigger the confirmation/signup template. It retains exact forwarded Preview origin, callback and `/portal`. Callback `error`/`error_code` inputs now return a sanitized sign-in failure before any code exchange or bootstrap; absent/invalid codes cannot establish a session. The active provider templates, Site URL, callbacks and production configuration were not mutated.

Post-failure state uses the immediately preceding protected verification (one tagged Sprint-owned A, no sign-in indicator, application/Storage zero) plus the accepted no-session failure, absence of Preview callback/bootstrap, unchanged protected ledger and no provider/application mutation. Sanitized state remains identity/tagged Auth `1`, ownership `sprint-owned`, application/profile/access `0`, owned Storage `0`, owned counts `0/1/0`; containment was not required. The protected secret is absent from Builder process state, so no second Admin query was fabricated and no operator action was requested.

Focused redirect, participant-helper, wrapper and dashboard tests pass; typecheck and lint pass. The production build passed using an isolated output directory because the pre-existing `.next` lint cache was locked; the temporary output/configuration was removed/reverted. JSON, encoding and diff checks pass. No further A retry is authorized until Architect reviews this diagnosis, the correction is deployed in a fresh exact-source alias-free Preview, the exact callback is rotated safely, and protected evidence establishes A is confirmation-state-ready without introducing a signup path.

### Plus-address identity boundary

Routing a controlled plus-address message into its base mailbox is normal delivery behavior and does not change the Auth identity, token or redirect. It is not treated as the primary defect. The protected helper now centralizes exact inbox matching: trim and lowercase only, with the full local part—including `+tag`—preserved. Deterministic no-secret coverage proves a full plus identity matches its case variant but does not match the base address or a different plus alias. No participant address, recipient header or message content is stored. A future protected check must return only sanitized exact-match classification, and the operator must privately confirm the newest recipient corresponds to the intended full identity. Participant execution remains stopped until a complete synthetic Preview journey passes.

### Exact-candidate Preview and prefetch boundary

Candidate `d9c020f2d09df5b59ce1a5e589bd68d8145cb1d0` deployed as `dpl_kKRfUc4Sy9zL4cw32rFm8C6DD27r`. Authoritative metadata reports Ready, Preview environment, exact `ppSourceSha`, alias array empty; the Vercel production build compiled, linted, typechecked and generated all routes. The old `ge5io3pa3` temporary callback was removed before exact `c16cgcork` callback addition. Dashboard verification shows exactly two allowlisted callbacks: unchanged production and exact new Preview. The old callback, other stale Preview callbacks and wildcards are absent; production Site URL is unchanged. Artifact-free callback remained on the new Preview and resolved to the `/portal` sign-in boundary. Production deployment and five stable aliases remain unchanged.

Synthetic delivered-flow proof cannot be claimed. The Builder environment has neither a controlled synthetic mailbox nor a protected Admin credential, and operator intervention is forbidden. The existing Magic Link template uses `ConfirmationURL`; Supabase documents that mail-security prefetch may consume this one-time action before the user. Therefore local source assertions, constructed URLs and Admin-generated links are insufficient, exactly as Architect specified. A prefetch-resistant path requires either email OTP or a two-step/interstitial action that defers verification until an explicit user action. Because both require changing the shared hosted Auth template and could affect production authentication, Builder stopped before provider mutation and returns for Architect review. No synthetic Auth/application/Storage state was created, so exact synthetic cleanup is `0/0/0`. No new A query, containment, participant message or B/C action occurred; final A retry is not demonstrably safe.
