# Precision Performance Roadmap

**Last reviewed:** 2026-08-02 after Sprint 035N

**Estimate:** about eight outcome sprints to Core Product Done.

## Executive summary

The accepted public release remains valid, but the trainer product is not complete. Sprint 034 established the clean, remotely backed-up source baseline. The next work is one trainer-visible Sprint 035 outcome; later work must establish domain and privacy authority, evidence handling, trends, timed field acceptance, operational ownership, and a final production Done decision.

## Readiness signals

| Signal | Status | Meaning |
|---|---|---|
| Public release | passed | Sprint 032 release remains accepted with limitations. |
| Auth/RLS foundation | passed | Sprint 021AH bounded synthetic proof is accepted. |
| Integrated lineage | passed | Sprint 034 reconciled and remotely backed up the governing baseline. |
| Core trainer value loop | attention | Human field use, approved outputs, evidence, trends, and ownership remain incomplete. |

## Roadmap

| Sprint | Phase | Outcome | Status | Dependency / exit signal |
|---|---|---|---|---|
| 034 | Restore trust | Reconciled clean product baseline and lifecycle ledger | done | Exact local/remote SHA equality and 16/16 validation groups pass |
| 035 | Prove access | First trainer access and core assigned-horse journey | attention | 035M candidate Ready; 035N found an unknown pre-existing alternate Preview callback and stopped clean before mutation or human acceptance |
| 036 | Authorize intelligence | Approved/versioned result formulas, thresholds, language and guidance | planned | Named domain/veterinary owner signs contract and fixtures pass |
| 037 | Govern evidence | Private uploads/evidence and lifecycle acceptance | planned | Privacy decision plus hosted denial/deletion/cleanup proof |
| 038 | Make history useful | Core trends and trainer review | planned | Approved rule semantics and accessible rendered acceptance |
| 039 | Prove the field workflow | Timed supported-device end-to-end trainer acceptance | planned | Under-60-second target and full journey pass |
| 040 | Make it operable | Named operations/privacy/support ownership and recovery rehearsal | planned | Owners accept runbooks; restore/rollback rehearsal passes |
| 041 | Release and decide | Integrated production release and Core Product Done acceptance | planned | Exact lineage, full acceptance matrix, deployment and rollback agree |

## Deferred Complete Vision

Voice, OCR/meter interpretation, commerce, enquiry delivery, advanced saved views, broader onboarding, CMS, and public expansion remain deferred unless explicitly promoted.

## Current action

**Do:** Reconcile ownership and lifecycle of the unknown pre-existing Preview callback, then authorize a clean exact-callback attempt and complete rendered/private trainer acceptance.

**Owner:** Architect / product owner.

**Decision:** Retain the existing alternate callback with a named owner or remove it as obsolete under explicit authority; do not infer cleanup or new mutation authority.
