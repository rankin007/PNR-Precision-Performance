Architect Pack 023H - Repository Lint Baseline Correction And Combined Closeout

Created: 2026-07-28
Workflow profile: strict
Architect outcome: Builder handoff to correct six pre-existing repository-wide ESLint errors with behavior-preserving, line-local changes, prove the affected tooling still behaves safely, and rerun/reconcile the combined Sprint 023E–023G closeout without changing product, migrations, dependencies or external state.

============================================================
FILE: planning/sprints/023H-repository-lint-baseline-correction-and-combined-closeout/requirements.md
============================================================
...
============================================================
FILE: planning/sprints/023H-repository-lint-baseline-correction-and-combined-closeout/blueprint.md
============================================================

# Sprint 023H - Repository Lint Baseline Correction And Combined Closeout Blueprint

## Phase 1 - Controlled Baseline And Exact Reproduction

1. Verify existing worktree, branch, baseline, clean index and complete 023E–023G attribution.
2. Verify the three targets have no existing sprint diff.
3. Reproduce exactly five intentional CommonJS rule errors and one unused type error with cache disabled.
4. Verify earlier focused/static/type/build gates and migration immutability.
5. Apply and verify all four Sprint 023H files.

Stop before editing if scope attribution or exact isolated root cause fails.

## Phase 2 - Minimal Lint Correction

1. Add one exact justified line-local suppression before each of five built-in-module `require()` calls.
2. Remove one unused type name from its import list.
3. Confirm no module, export, runtime, filesystem, network, validation or recommendation logic changed.
4. Inspect the three-file diff before testing.

## Phase 3 - Tool Behavior Proof

1. Run syntax checks for both CommonJS tools.
2. Prove Pack check and dry-run/no-write behavior.
3. Prove representative Pack path/delimiter failures remain safe.
4. Require update-method without network/write/run side effects and test pure exports.
5. Run recommendation validation.
6. Prove suppression count/scope and temporary cleanup.

## Phase 4 - Combined Revalidation

1. Run full repository ESLint without stale-cache ambiguity.
2. Rerun every focused and maintained 023E–023G gate.
3. Run TypeScript, build, approved-path, dependency, privacy/secret, migration and diff checks.
4. Preserve exact local/structural/deferred proof distinctions.
5. Reconcile only authorised earlier evidence/outcomes when fully supported.

## Phase 5 - Closeout

1. Produce all four 023H reviews.
2. Update planning state and reserve Sprint 023I for remote/provider proof.
3. Finish with exactly one allowed outcome.
4. Leave combined work unstaged/uncommitted unless separately instructed.
5. Do not begin Sprint 023I.

============================================================
FILE: planning/sprints/023H-repository-lint-baseline-correction-and-combined-closeout/acceptance.md
============================================================
...
============================================================
FILE: planning/sprints/023H-repository-lint-baseline-correction-and-combined-closeout/handoff-prompt.md
============================================================

# Sprint 023H - Builder Handoff Prompt

You are Builder for Sprint 023H - Repository Lint Baseline Correction And Combined Closeout.

Apply and verify all four Sprint 023H files in the existing intentionally uncommitted worktree `C:\tmp\pnr-023e-local-upload-storage` on branch `codex/023E-local-upload-and-storage-implementation-and-proof`. Do not create another branch/worktree.

First prove complete attribution, clean index, no earlier diff in the three lint targets, unchanged migrations/dependencies, and exactly five intentional CommonJS import errors plus one unused type error. Stop before editing if the root cause or scope differs.

Make only the narrow correction: add one justified line-local `@typescript-eslint/no-require-imports` suppression immediately before each existing built-in `require()` and remove only the unused `BiochemistryRecommendationCategory` type import. Preserve CommonJS, filenames, package type, exports, `require.main`, behavior and every recommendation assertion. Do not use broad lint configuration or convert modules.

Add network-free behavior proof for syntax, Pack check/dry-run/safe failures, update-method require/pure exports, recommendation validation and suppression scope. Never execute the live updater network/write path.

Run full repository ESLint without stale-cache ambiguity and every combined 023E–023G gate. Reconcile earlier outcomes only if all evidence supports it, preserving all executable database, remote/provider and production deferrals.

Do not modify product/migrations, install dependencies, inspect protected configuration, apply migrations, create Storage, contact providers, invoke the live updater, deploy, stage, commit, push, merge or begin Sprint 023I. Finish with one allowed outcome and leave all combined work uncommitted unless separately instructed.
