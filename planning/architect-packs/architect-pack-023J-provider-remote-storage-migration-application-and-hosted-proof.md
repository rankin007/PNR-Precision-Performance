Architect Pack 023J - Provider, Remote Storage, Migration Application And Hosted Proof

Created: 2026-07-28
Workflow profile: strict
Architect outcome: Builder handoff to complete the missing hosted evidence contract locally, establish a separately committed clean candidate, then apply and prove it only in an explicitly designated non-production Australian Supabase/Vercel environment using synthetic data and fail-closed safety behavior. Production rollout and real safety-provider availability remain later work.

============================================================
FILE: planning/sprints/023J-provider-remote-storage-migration-application-and-hosted-proof/requirements.md
============================================================
...
- Supabase project region is Sydney `ap-southeast-2`; `ap-southeast-1` is Singapore and fails this gate;
...
- no Australian-storage or recovery-readiness claim is made beyond retained evidence.
...
Retain sanitised provider/region/recovery evidence in `planning/reviews/023J-provider-region-and-recovery-decision.md` or use the Manual Intervention Rule. If Sydney region, provider suitability or Storage-object recovery is not approved/provable, stop `provider-region-recovery-blocked-clean` before remote mutation.
...
- claiming legal certification, Australian/recovery production readiness or complete Sprint 023 delivery.
...
============================================================
FILE: planning/sprints/023J-provider-remote-storage-migration-application-and-hosted-proof/blueprint.md
============================================================

# Sprint 023J - Provider, Remote Storage, Migration Application And Hosted Proof Blueprint

## Phase 1 - Clean Commit And Isolated Baseline

1. Require the separately created clean combined 023E–023I commit.
2. Verify ancestry, paths, reconciled outcomes, ledger and clean status.
3. Create isolated 023J branch/worktree from the exact commit.
4. Apply and verify all four 023J sprint files.
5. Stop cleanly if no valid commit exists.

## Phase 2 - Authority, Provider And Read-Only Preflight

1. Record exact non-production Supabase/Vercel targets, operator and mutation allowlist.
2. Verify Sydney region, provider/privacy suitability and Storage-object recovery approval.
3. Recheck official primary documentation.
4. Perform count-only target/ledger/schema/conflict/legacy/zero-state preflight.
5. Stop before mutation on any ambiguity, real data, discrepancy, unknown object or recovery gap.

## Phase 3 - Local Contract Completion

1. Add migration `0019` with exact RPC, permission, bucket and Storage-policy contract.
2. Complete signed-direct transfer/finalisation UI and server flow while preserving fail-closed safety.
3. Add focused SQL/RLS/action/UI/harness tests and synthetic fixtures.
4. Align candidate-ledger validators through `0019` with adversarial proof.
5. Run complete local validation and reparse-safe build.
6. Stop `remote-candidate-ready-for-commit` for separate commit instruction.

## Phase 4 - Clean Candidate Commit Gate

1. After separate instruction, stage only approved 023J files.
2. Review staged manifest/diff and rerun integrity checks.
3. Commit locally and verify exact clean SHA.
4. Invalidate remote progression if any source changes afterward.

## Phase 5 - Remote Migration And Storage

1. Repeat exact identity/region/ledger/inventory/conflict preflight.
2. Capture sanitised recovery/pre-apply evidence.
3. Apply only `0018` and `0019` through supported migration tooling.
4. Stop on any failure without repair/manual continuation.
5. Verify ledger, schema, RLS, grants, bucket and policies.
6. Prove anonymous/ordinary direct denials and fail-closed legacy treatment.

## Phase 6 - Protected Configuration And Hosted Preview

1. Configure only matching protected non-production values.
2. Create/configure `CRON_SECRET` without exposing it.
3. Deploy exact clean commit only to named preview/non-production target.
4. Verify no production alias/deployment mutation.
5. Manually prove Cron route; do not activate production schedule.

## Phase 7 - Synthetic Hosted Matrix And Recovery

1. Create bounded synthetic actors/scopes/test/files.
2. Run accepted/denied role, forgery, limit, direct Storage and fail-closed lifecycle matrix.
3. Prove idempotency, replacement failure, reconciliation overlap and audit redaction.
4. Prove one approved synthetic Storage-object recovery cycle without availability.
5. Clean exact owned fixtures Auth-last and prove final `0/0/0`.

## Phase 8 - Closeout

1. Verify rollback/containment and production non-mutation.
2. Produce all eight reviews and update planning truth.
3. Finish with exactly one allowed outcome.
4. Do not push/merge/promote unless separately instructed.

============================================================
FILE: planning/sprints/023J-provider-remote-storage-migration-application-and-hosted-proof/acceptance.md
============================================================
...
- [ ] Separate clean combined 023E–023I commit exists and exact SHA is recorded.
- [ ] Required ancestry, reconciled outcomes, committed ledger `0001–0018` and clean status pass.
- [ ] Isolated 023J branch/worktree starts from that exact commit.
- [ ] Original and earlier sprint worktrees remain unchanged.

## Authority, Provider And Preflight

- [ ] Exact non-production Supabase/Vercel target/operator/mutation allowlist is recorded.
- [ ] Target is demonstrably non-production, synthetic-only and not connected to production aliases/data.
- [ ] Supabase is verified Sydney `ap-southeast-2` through sanitised direct evidence.
- [ ] Aprec8 approves current provider/DPA/subprocessor/overseas-access boundary for this proof.
- [ ] Storage-object backup/recovery method and retention/expiry are approved and testable.
- [ ] Current official primary sources and access dates support platform assumptions.
- [ ] Remote ledger is exactly `0001–0017`; no partial/conflicting 023E objects exist.
- [ ] Count-only legacy inventory passes migration `0018` gates without exposing payloads.
- [ ] No real/customer/production data is present and initial synthetic counts are governed.

## Local Completion And Commit

- [ ] Migration `0019` additively defines exact RPCs, RLS/grants, bucket configuration and Storage policies.
- [ ] Migrations `0001–0018` remain unchanged.
- [ ] Signed-direct upload/finalisation is implemented with server authority and no overwrite.
- [ ] Default hosted safety remains fail closed; CSV remains disabled.
- [ ] Focused RPC/RLS/Storage/action/UI/Cron tests and full local matrix pass.
- [ ] Candidate ledger validates exactly `0001–0019` and rejects gaps/duplicates/`0020+`.
- [ ] TypeScript, full ESLint, static/local gates and reparse-safe build pass.
...
- [ ] Eight required reviews distinguish local, committed, remotely applied, hosted and deferred proof.
- [ ] No real data, live malware, provider secret, raw object key, signed URL or payload enters evidence/logs.
- [ ] Production Supabase/Vercel/DNS/domain/alias/environment/data remains unchanged.
- [ ] No dependency/provider addition, CSV enablement, real safety adapter, public launch or unrelated change occurs.
...
- `provider-region-recovery-blocked-clean`: Sydney region, provider suitability or Storage-object recovery is not approved/proven; no external mutation occurs.
...
============================================================
FILE: planning/sprints/023J-provider-remote-storage-migration-application-and-hosted-proof/handoff-prompt.md
============================================================

# Sprint 023J - Builder Handoff Prompt

You are Builder for Sprint 023J - Provider, Remote Storage, Migration Application And Hosted Proof.

Apply and execute from these four sprint files only after a separate clean combined Sprint 023E–023I commit exists. If no exact clean SHA has been separately created/reported, stop `remote-baseline-commit-blocked-clean`; never use the uncommitted 023E worktree for remote work.

After the baseline exists, create an isolated 023J branch/worktree. Before any external mutation, establish the exact authorised non-production Supabase/Vercel target, Sydney `ap-southeast-2` region, synthetic-only data classification, provider/privacy suitability, Storage-object recovery method, operator, rollback and mutation allowlist. Perform count-only ledger/schema/legacy/conflict/zero-state preflight. Stop on ambiguity, real data, discrepancy or missing recovery.

The current candidate is not remotely ready: migration `0018` lacks the RPCs and executable Storage policy called by source, and the UI does not transfer bytes. Complete this additively in migration `0019` and the narrow approved evidence source. Preserve fail-closed scanner/sanitiser behavior and disabled CSV. Add complete local tests, align candidate-ledger validators through `0019`, and run full validation/build.

Then stop `remote-candidate-ready-for-commit` for a separate commit instruction. Do not apply or deploy uncommitted source. After the clean 023J commit exists, rerun every target/preflight gate and apply only committed `0018–0019` to the exact non-production project. Never reset/repair/force or manually continue a failed migration.

Configure protected values only in the named non-production hosted target, deploy only the exact commit to preview/non-production, and do not move production aliases or activate production Cron. Run synthetic role/scope/Storage/lifecycle/Cron/recovery proof. Evidence must remain unavailable without real approved safety adapters. Clean exact owned fixtures Auth-last and prove final `0/0/0`.

Do not expose secrets, use real data/live malware, enable CSV, add providers/dependencies, alter production, push/merge/promote or claim production readiness. Finish with one allowed outcome and complete the five-part Manual Intervention Rule for every blocker.
