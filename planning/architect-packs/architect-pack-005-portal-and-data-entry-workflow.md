...
Sprint 005 is approved as a portal and data-entry workflow sprint. The goal is to make the core operational workflow usable end-to-end: assigned horse list/detail behavior, daily record creation, feeding log creation, track session creation, submission review/correction, validation rules, user-facing error states, and phone/desktop smoke evidence.

Live Supabase test-user and direct remote RLS verification remain blocked by missing configured Supabase environment/test-user access unless the user provides non-secret access/setup before or during Sprint 005.

The approved path to Done remains Sprints 005-007 in `planning/SPRINT_SCHEDULE.md`.
...
Reason: this is an existing web application with Supabase, Stripe, auth, data-entry, and deployment surfaces. Sprint 005 touches member portal and operational data-entry workflows using protected Supabase data, so Builder must stay inside the approved sprint scope, avoid credential exposure, avoid destructive database/data changes, and stop before commerce, production deployment, broad schema redesign, or production project-setting changes.
...
Builder may edit files inside the Sprint 005 approved scope without another approval.

Sprint 005 explicitly authorizes narrow implementation work for:

- assigned horse list/detail data behavior under existing portal access and RLS boundaries
- daily record, feeding log, and track session creation through existing ops/data-entry surfaces
- submission review and correction behavior for those operational records
- validation rules and user-facing error states for the approved forms
- permission-safe operational route guards if the existing signed-in-only guard is insufficient
- RLS policy fixes only where directly required to enforce assigned-horse and horse-record write boundaries for Sprint 005 workflows
- non-secret documentation, validation notes, and smoke scripts/checklists

Builder must stop and ask before:

- touching or printing secret values, tokens, credentials, private keys, passwords, full connection strings, or secret fragments
...
- changing database schema beyond the smallest additive or policy-focused Sprint 005 workflow fix
...
- changing admin commerce management, launch infrastructure, Node runtime compatibility, or broad cleanup outside this sprint
...
`planning/sprints/005-portal-and-data-entry-workflow/`
...
2. Builder reads `planning/sprints/005-portal-and-data-entry-workflow/SPRINT.md`.
3. Builder maps the existing portal horse and ops/data-entry routes before editing.
4. Builder carries forward Sprint 004 live Supabase/RLS blockers and requests only non-secret setup/access if live verification is possible.
5. Builder implements the smallest workflow fixes needed for assigned horses, daily records, feeding logs, track sessions, and submission correction.
6. Builder validates with lint, TypeScript, build, secret-fragment scans, and manual phone/desktop smoke evidence or clearly flagged manual-intervention instructions.
7. Builder refreshes planning/docs at close and hands off to Sprint 006.

---

## Blockers

No blocker for Sprint 005 planning.

Builder may discover that real Supabase test users, assigned horse fixtures, remote RLS execution, or browser/device access are unavailable. If so, Builder must still inspect and harden local code paths where evidence allows, then flag the blocked acceptance case with exact manual-intervention instructions and the smallest non-secret user action needed to complete it.

============================================================
FILE: planning/sprints/005-portal-and-data-entry-workflow/SPRINT.md
============================================================
...

{
  ...
  "phase": "apply-pack",
  "sprint": "005-portal-and-data-entry-workflow",
  "updated": "2026-07-11T18:30:00+10:00"
}
