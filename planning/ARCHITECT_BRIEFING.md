...
Incident-safe follow-up confirmed a source defect: all Supabase OTP request errors are masked as apparent send success. Read-only provider diagnosis then stopped when the log explorer unexpectedly emitted a protected Auth identifier in tool output. The value was not inspected or reused, browser state was cleared, and no authentication or external/source mutation occurred. Provider disposition and cooldown causality remain unproved. Rollback is not ready: the old deployment expects Magic Link behavior, the exact historical template representation is unavailable, and no separate forward OTP-compatible pair has been proven.

Architect-authorized continuation completed the in-scope correction without raw logs or authentication. Candidate `7d12e0d229324d8b07bcff7cb76bcc11f4d6477a` makes accepted/missing-identity requests non-definitive and returns generic retry-later for allowlisted operational rejection, with no raw detail. The no-email Admin preparation helper and fail-closed diagnostic sanitizer pass deterministic tests. Exact Preview `dpl_CiYWswcayHztGRHEUUFsZmQkTDie` and corrected production `dpl_9zVS8HSujThkFTP3hisyfBVknKWb` are Ready; all five aliases are reconciled. Preceding OTP-capable production `dpl_EUJyBLQp7okgbPwtBU6nmMH88L6A` is the forward rollback. The unchanged current OTP template is exactly restorable from protected storage and fingerprinted `23c6a254...`; Site URL and production/corrected-Preview callbacks are exact. Production request/protected-route availability is proven without email, but delivery/session and provider disposition are unproved. With the configured two-emails/hour ceiling, another synthetic attempt requires Architect review and a fresh rate-limit window. A/B/C remain unstarted.

Manual-preview incident cleanup is now exact-owned `0/0/0`. The exact Preview-origin session was signed out first; eight application/profile/access/fixture records were removed dependency-safely; Storage was zero; Auth was deleted last. Approved-project Auth count is three: Participant A remains preserved and two identities remain ambiguous, including the operator incident identity after a protected zero-match attempt. Complete Vercel inventory proves five-of-five production aliases; the apparent missing alias was only omitted by the prior inspection view. Exact manual Preview remains Ready and alias-free. Site URL, two callbacks, OTP-template fingerprint and forward rollback are unchanged. GitHub proves local/remote equality at `494d0db598a451cebf0aa985fe5f9f602cfdb513`. No email, OTP, A/B/C or provider mutation occurred.
