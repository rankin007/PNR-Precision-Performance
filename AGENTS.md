...
Inside an active 120x Fly, Pack delivery still ends the Architect phase and the
Builder still applies the Pack. The detailed Fly doctrine governs the subsequent
pre-code transition: `local` and `cross-layer` flights proceed from the human's
`go` plus the applied Pack and receive their required independent review after
implementation; `critical` flights require a fresh Architect plan pass before
code; Fly Lean uses its persistent Architect plan pass. No second human prompt is
added. This exception ends with the one Pack and sprint created in that flight.
