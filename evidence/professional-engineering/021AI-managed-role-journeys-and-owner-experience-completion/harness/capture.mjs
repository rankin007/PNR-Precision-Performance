...
const harness = path.join(root, "evidence/professional-engineering/021AI-managed-role-journeys-and-owner-experience-completion/harness");
const visualDir = path.join(root, "evidence/professional-engineering/021AI-managed-role-journeys-and-owner-experience-completion/visuals");
const port = 3219;
...
const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "021ai-chromium-"));
...
const auditExpression = (expectedSections, namedViewport) => `(() => {
  ...
  const targets = [...document.querySelectorAll(interactiveSelector)]
    ...
    .map((element) => {
      ...
      const nativeFocus = ["INPUT", "SELECT", "TEXTAREA"].includes(element.tagName) || element.hasAttribute("tabindex");
      return {
        selector: selectorFor(element),
        accessibleName,
...
const captures = [
  { name: "trainer-managed-access-mobile", mode: "access", file: "01-mobile-trainer-managed-access-414x896.png", expected: ["#assign-access-heading"] },
  { name: "managed-role-permissions-mobile", mode: "permissions", file: "02-mobile-managed-role-permissions-414x896.png", expected: ["#assign-access-heading"] },

  { name: "owner-latest-result-mobile", mode: "owner", file: "03-mobile-owner-latest-result-414x896.png", expected: ['a[href="/?mode=trends"]'] },
  { name: "owner-exact-horse-trends-mobile", mode: "trends", file: "04-mobile-owner-exact-horse-trends-414x896.png", expected: ["#trend-controls"] },
  { name: "managed-access-owner-overview-desktop", mode: "desktop", file: "05-desktop-managed-access-owner-overview-1440x900.png", expected: ["#assign-access-heading", 'a[href="/?mode=trends"]', "#trend-controls"], desktop: true },
];
...
function writeManifest() {
  ...
  const manifest = {
    ...
    privacy: "Synthetic data only; no real identity, horse, stable, email, note, credential, token, provider identifier or confidential record.",
  };
...
try {
  ...
  fs.writeFileSync(path.join(visualDir, "geometry.json"), JSON.stringify(geometry, null, 2) + "\n");
  writeManifest();
  console.log("Sprint 021AI geometry and full-page captures passed: 13/13 runs, 5/5 images.");
} finally {
