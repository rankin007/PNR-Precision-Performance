...
`$120x-builder`: the human approves the exact implementation plan. Inside an
active `/fly` or `$120x-fly` after the human says `go`, the detailed Fly
doctrine controls the class-scaled authority transition: `local` and
`cross-layer` flights build from the applied Pack and receive their required
independent review after implementation; `critical` flights require a genuinely
fresh Architect `pass` on the exact Builder plan before code; Fly Lean uses its
persistent Architect plan pass. No second human prompt is required, and that
authority ends with the one Pack and sprint created in that flight.
