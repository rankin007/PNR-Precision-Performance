...
**The class-scaled Fly exception.** The normal code gate above remains mandatory
for standalone `/build` and `$120x-builder`. Inside an active `/fly` or
`$120x-fly` after the human has said `go`, the detailed Fly doctrine controls
the authority transition: `local` and `cross-layer` flights build from the
applied Pack and receive their required independent review after implementation;
`critical` flights require a genuinely fresh Architect `pass` on the exact
Builder plan before code; Fly Lean uses its persistent Architect plan pass.
Implementation then proceeds without a second human prompt. That authority ends
