...
implementation diff after the build. Stop here and continue at "Build under
class-scaled authority."**
...
Inside a `critical` Fly only, fresh Architect `pass` is approval of that exact
Builder plan. On a `local` or `cross-layer` Fly, the human's `go` plus the
applied sprint authority permits implementation and the class-required review
follows the build. Proceed automatically in either case. Do not ask the human to
type `approved` after `go`.
...
## Build under class-scaled authority

Give the Builder the applied sprint files. On a `critical` flight, also give it
the exact passed plan.

The Builder:

- implements only the applied sprint authority and, on a `critical` flight, the
  passed plan;
...
If implementation must differ from the applied sprint authority, stop for a Pack
correction or scope decision. On a `critical` flight, any change from the passed
plan also returns to planning and a new fresh Architect review before that change.
